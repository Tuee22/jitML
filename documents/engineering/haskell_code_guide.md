...
| Surface | Invariants | Owning module |
|---------|------------|---------------|
| `RawSupervisedRuntime` → refined supervised runtime | One topology-complete graph, exact transforms/decoder, checked graph-ordered virtual parameter slices, and one physical flat weight tensor; unsupported or inconsistent values fail refinement | `src/JitML/SL/RuntimeArtifact.hs` |
| `TrainingMetrics` | A successful training return carries the exact runtime program, canonical initial/final JMW1 bytes, verified dataset digest, a finite held-out parity probe, and the trainer-owned successful mini-batch count; no caller may reconstruct an observation from the plan | `src/JitML/SL/TrainingExecution.hs` with loop-owned counters in `Architecture.hs` / `Regression.hs` |
| Supervised V2 construction and reload | Product construction binds the authoritative row projection; generic construction binds the composite `(rowId, canonicalPlanTransport)` origin, and `PlanId` alone is insufficient. Both rederive the canonical dataset-read digest, bind exact runtime/JMW1 bytes and observed updates, and reload only from the persisted graph and physical bytes. Product completion additionally requires exactly four finite metric rows in canonical order: `train_loss`, `validation_loss`, `examples_processed`, and the authoritative held-out metric. | `src/JitML/SL/RuntimeArtifact.hs`, `src/JitML/Checkpoint/{Writer,Format,Store,WeightCodec}.hs`, `src/JitML/Product/Publisher.hs` |
| Completed V2 pointer publication | A writer reads the current latest-pointer ETag, uses that exact expectation for compare-and-swap, and may publish a completed-checkpoint event only after Store returns `PointerWritten manifestSha` for the exact stored manifest. Conflict or acknowledgement of another manifest is a typed non-publication. | `src/JitML/Checkpoint/{Writer,Store}.hs` and supervised publication call sites |
| Candidate/completed persistence split | Candidate writers take no completion proof, return opaque `StoredCandidateCheckpoint`, and never publish `latest`. Completed writers take `CompletedTraining` directly and return opaque `StoredCompletedCheckpoint` only after exact pointer CAS adoption; no completed persistence/event boundary carries `Maybe CompletedTraining`. Immutable create conflict is accepted only after an exact read proves byte equality. Local `CheckpointWriteError` distinguishes invalid input, object conflict, pointer conflict, and filesystem failure; MinIO retains typed `ServiceError`. | `src/JitML/Checkpoint/{Writer,Store}.hs` and training/RL publication call sites |
| Exact persisted admission | `admitLatestCheckpoint` reads `P1`, verifies the exact addressed V2 outer/body manifest, requires exact `P1 == P2`, and only then independently fetches and binds every physical blob. `admitCheckpointAt` performs the same manifest/blob binding without pointer reads. Only `requireAdmittedCompletedCheckpoint` can turn opaque `AdmittedCheckpoint` into opaque `AdmittedCompletedCheckpoint`; decoded Format values are structural data, not persisted proof. | `src/JitML/Checkpoint/Store.hs` |
...
| Selected structural execution | CPU/CUDA load generated status-returning ABI-version/capability-checked libraries; Metal verifies exact generated metadata and dispatches its MSL through the fixed bridge. Compile, load, symbol, ABI, capability, contract, and execution failures remain typed | `src/JitML/Engines/RuntimeOperations{Device,Cuda,Metal}.hs`, generated by `src/JitML/Codegen/RuntimeOperations{Cpu,Cuda,Metal}.hs` |
