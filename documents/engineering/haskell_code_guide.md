...
| Typed-graph training and serving | Supervised training updates the canonical `LayerGraph` through the oneDNN device path on every substrate; see [numerical_core.md](numerical_core.md#execution-boundary) for the per-substrate status. Serving reconstructs and refines the admitted graph, applies transforms outside it, and calls the shared pure `runLayerGraph`; the deleted `RuntimeOperations*` structural ABI is not an executor or fallback. `[LayerSpec]` / `[LayerState]` remain only as the Dense-family initialization adapter. | `src/JitML/SL/Architecture.hs`, `src/JitML/Numerics/{LayerGraph,LayerGraphOneDnn}.hs`, `src/JitML/SL/RuntimeArtifact.hs`, `src/JitML/Checkpoint/Store.hs` |
