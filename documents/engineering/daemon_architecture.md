...
Per doctrine `Long-Running Daemons in the Same Binary`:
...
| Endpoint | Behaviour |
|----------|-----------|
| `/healthz` | `200 OK` if alive; `500` otherwise |
| `/readyz` | `200 OK` only after `Ready` lifecycle phase; `503` otherwise |
| `/metrics` | Prometheus format; scraped by kube-prometheus-stack |

Logging: structured JSON on stderr with fields `ts`, `level`, `msg`,
...
The consumer subscribes to the substrate-scoped command topics (training,
tune, RL, inference, plus `inference.command.apple-silicon` on the host
daemon).
...
_(Generated by Sprint 5.3; the table above is the authoritative content
until generation lands.)_
