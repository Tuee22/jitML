...
| [jit_codegen_architecture.md](jit_codegen_architecture.md) | Content-addressed cache, per-substrate compilers (Metal/oneDNN/CUDA), local kernel handle/envelope surface, Linux CPU libdnnl-linked FFI and `HasEngine` execution, guarded CUDA FFI runner boundary, Apple Silicon hybrid pattern, hardware auto-tuning, target Metal/live CUDA validation boundary; the mandatory Tart-VM Swift/Metal build boundary on Apple Silicon (host keeps only the Metal framework + the VM-produced `.dylib`; full Xcode is never installed on the host) |
