...
| [jit_codegen_architecture.md](jit_codegen_architecture.md) | Content-addressed cache, per-substrate compilers (Metal/oneDNN/CUDA), local kernel handle/envelope surface, Linux CPU libdnnl-linked FFI and `HasEngine` execution, guarded CUDA FFI runner boundary, Apple Silicon Tart-VM build Metal JIT, hardware auto-tuning, target Metal/live CUDA validation boundary; on Apple Silicon the `jitml`-managed Tart VM builds the Swift glue dylib with `swift build`, the dylib is copied out to the host, and the host JIT-compiles the Metal shader at load via `MTLDevice.makeLibrary(source:)` and executes on the Metal GPU |
| [apple_silicon_metal_headless_builds.md](apple_silicon_metal_headless_builds.md) | Target true-headless Apple Silicon Metal JIT architecture: fixed host bridge, runtime MSL compilation via `MTLDevice.makeLibrary(source:options:)`, source/metadata cache artifacts, optional Swift JIT lane, and rationale for rejecting Tart, full Xcode, offline `.metallib`, and per-cache-miss Swift builds |
