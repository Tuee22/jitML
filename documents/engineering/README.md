...
| [unit_testing_policy.md](unit_testing_policy.md) | Testing Doctrine; Standard Testing Stack; Test Categories; Test Organization | Eight jitML test stanzas including local SL/RL/HPO/backends Integration extensions, Linux CPU generated-kernel execution, current `jitml-e2e` scaffold/live-plan surface, and the closed no-caveat Playwright product matrix |
...
| [purescript_frontend.md](purescript_frontend.md) | Current PureScript shell, generated contracts, panel/demo-route metadata, `spec-node` smoke-test runner, demo shim, Playwright scaffold, typed live-plan step, and closed no-caveat browser/animation/replay/Playwright product matrix |
