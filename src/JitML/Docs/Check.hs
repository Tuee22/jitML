...
import JitML.Generated.Registry (GeneratedSectionRule (..), endMarker, generatedSectionRules, startMarker)
...
replaceGeneratedSection rule current = do
    startIndex <- maybe (Left "start marker is missing") Right (findIndex ((== startMarker (ruleKey rule)) . Text.strip) currentLines)
    endIndex <- maybe (Left "end marker is missing") Right (findIndex ((== endMarker (ruleKey rule)) . Text.strip) currentLines)
