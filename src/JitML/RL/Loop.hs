...
import JitML.RL.Buffer (BufferKind (..), ReplayBuffer, emptyBuffer)
  ...
  let episodes = rlEpisodes loop
      ...
      buffer =
        emptyBuffer
          ...
          (rlBufferCapacity (loopConfig loop))
   in RLLoopResult
...
runOneEpisode loop episodeId =
  let environment = loopEnvironment loop
      ...
      walk acc stepIx
        | stepIx >= maxSteps = finalize acc stepIx False
        | otherwise =
            let action = (stepIx + episodeId) `mod` max 1 (environmentActionCount environment)
             ...
             in if stepDone step
                  then finalize acc' (stepIx + 1) True
                  else walk acc' (stepIx + 1)
      finalize acc steps done =
        EpisodeResult
