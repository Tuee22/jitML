...
buildPrerequisitePlan prerequisites root =
    case transitiveClosure prerequisites root of
        ...
        Right closure -> do
            steps <- mapMaybe id <$> traverse missingStep closure
            pure
                ( Right
                    PrerequisitePlan
...
renderStep step =
    ...
    , "    remedy: " <> maybe "(none)" id (prerequisiteStepRemedyHint step)
...
applyPrerequisitePlan :: SubprocessEnv -> [Prerequisite] -> PrerequisitePlan -> IO (Either PrerequisitePlanError ())
                ...
                pure $
                    Left
                        ( PrerequisitePlanMissingRemediation
                            ...
                            (maybe "(none)" id (prerequisiteStepRemedyHint step))
                        )
