...
-- Dense and spatial-convolution production callbacks dispatch one thread per
-- independent result element while preserving explicit ascending-index order
-- inside every reduction.  The complete-operator opcodes remain a serial
-- reference floor for the rest of the 'LayerOp' vocabulary, keeping all work on
-- the host GPU without checking in native kernel sources or substituting the
-- pure Haskell oracle.
...
metalLayerTrainingMetadata =
  Text.unlines
    ...
    , "  \"launch_policy\": \"staged-element-parallel-ascending-inner-reductions\","
...
metalLayerTrainingProgram =
  Text.unlines
    ...
    , "}"
    , ""
    , "inline void jitml_dense_forward_element("
    , "    device float *out, const device float *x, const device float *weights,"
    , "    const device float *bias, int inputs, int outputs, int batch, uint gid) {"
    , "  int q=int(gid), total=batch*outputs; if(q>=total)return; int n=q/outputs,o=q%outputs;"
    , "  float acc=bias[o]; for(int i=0;i<inputs;++i)acc+=weights[o*inputs+i]*x[n*inputs+i]; out[q]=acc;"
    , "}"
    , "inline void jitml_dense_backward_data_element("
    , "    device float *dx, const device float *weights, const device float *dy,"
    , "    int inputs, int outputs, int batch, uint gid) {"
    , "  int q=int(gid), total=batch*inputs; if(q>=total)return; int n=q/inputs,i=q%inputs;"
    , "  float acc=0.0f; for(int o=0;o<outputs;++o)acc+=weights[o*inputs+i]*dy[n*outputs+o]; dx[q]=acc;"
    , "}"
    , "inline void jitml_dense_backward_weights_element("
    , "    device float *dw, device float *db, const device float *x, const device float *dy,"
    , "    int inputs, int outputs, int batch, uint gid) {"
    , "  int q=int(gid), wc=outputs*inputs; if(q<wc){int o=q/inputs,i=q%inputs;float acc=0.0f;"
    , "    for(int n=0;n<batch;++n)acc+=dy[n*outputs+o]*x[n*inputs+i];dw[q]=acc;return;}"
    , "  int o=q-wc;if(o<outputs){float acc=0.0f;for(int n=0;n<batch;++n)acc+=dy[n*outputs+o];db[o]=acc;}"
    , "}"
    , ""
    ...
    , "}"
    , ""
    , "inline void jitml_conv2d_forward_element(device float *out,const device float *x,const device float *weights,const device float *bias,const device int *g,uint gid){"
    , "  int cin=g[0],cout=g[1],h=g[2],w=g[3],kh=g[4],kw=g[5],sh=g[6],sw=g[7],ph=g[8],pw=g[9],batch=g[10];int oh=(h+2*ph-kh)/sh+1,ow=(w+2*pw-kw)/sw+1;"
    , "  int q=int(gid),total=batch*cout*oh*ow;if(q>=total)return;int ox=q%ow;q/=ow;int oy=q%oh;q/=oh;int co=q%cout;int n=q/cout;float acc=bias[co];"
    , "  for(int ci=0;ci<cin;++ci)for(int ky=0;ky<kh;++ky)for(int kx=0;kx<kw;++kx){int iy=oy*sh+ky-ph,ix=ox*sw+kx-pw;if(iy>=0&&iy<h&&ix>=0&&ix<w)acc+=x[((n*cin+ci)*h+iy)*w+ix]*weights[((co*cin+ci)*kh+ky)*kw+kx];}out[int(gid)]=acc;"
    , "}"
    , "inline void jitml_conv2d_backward_data_element(device float *dx,const device float *weights,const device float *dy,const device int *g,uint gid){"
    , "  int cin=g[0],cout=g[1],h=g[2],w=g[3],kh=g[4],kw=g[5],sh=g[6],sw=g[7],ph=g[8],pw=g[9],batch=g[10];int oh=(h+2*ph-kh)/sh+1,ow=(w+2*pw-kw)/sw+1;"
    , "  int q=int(gid),total=batch*cin*h*w;if(q>=total)return;int ix=q%w;q/=w;int iy=q%h;q/=h;int ci=q%cin;int n=q/cin;float acc=0.0f;"
    , "  for(int co=0;co<cout;++co)for(int ky=0;ky<kh;++ky)for(int kx=0;kx<kw;++kx){int yn=iy+ph-ky,xn=ix+pw-kx;if(yn>=0&&xn>=0&&yn%sh==0&&xn%sw==0){int oy=yn/sh,ox=xn/sw;if(oy<oh&&ox<ow)acc+=weights[((co*cin+ci)*kh+ky)*kw+kx]*dy[((n*cout+co)*oh+oy)*ow+ox];}}dx[int(gid)]=acc;"
    , "}"
    , "inline void jitml_conv2d_backward_weights_element(device float *dw,device float *db,const device float *x,const device float *dy,const device int *g,uint gid){"
    , "  int cin=g[0],cout=g[1],h=g[2],w=g[3],kh=g[4],kw=g[5],sh=g[6],sw=g[7],ph=g[8],pw=g[9],batch=g[10];int oh=(h+2*ph-kh)/sh+1,ow=(w+2*pw-kw)/sw+1,wc=cout*cin*kh*kw;int q=int(gid);"
    , "  if(q<wc){int kx=q%kw;q/=kw;int ky=q%kh;q/=kh;int ci=q%cin;int co=q/cin;float acc=0.0f;for(int n=0;n<batch;++n)for(int oy=0;oy<oh;++oy)for(int ox=0;ox<ow;++ox){int iy=oy*sh+ky-ph,ix=ox*sw+kx-pw;if(iy>=0&&iy<h&&ix>=0&&ix<w)acc+=dy[((n*cout+co)*oh+oy)*ow+ox]*x[((n*cin+ci)*h+iy)*w+ix];}dw[int(gid)]=acc;return;}"
    , "  int co=q-wc;if(co<cout){float acc=0.0f;for(int n=0;n<batch;++n)for(int oy=0;oy<oh;++oy)for(int ox=0;ox<ow;++ox)acc+=dy[((n*cout+co)*oh+oy)*ow+ox];db[co]=acc;}"
    , "}"
    , ""
    ...
    , "}"
    , ""
    , "inline void jitml_conv3d_forward_element(device float *out,const device float *x,const device float *weights,const device float *bias,const device int *g,uint gid){"
    , "  int cin=g[0],cout=g[1],d=g[2],h=g[3],w=g[4],kd=g[5],kh=g[6],kw=g[7],sd=g[8],sh=g[9],sw=g[10],pd=g[11],ph=g[12],pw=g[13],batch=g[14];int od=(d+2*pd-kd)/sd+1,oh=(h+2*ph-kh)/sh+1,ow=(w+2*pw-kw)/sw+1;"
    , "  int q=int(gid),total=batch*cout*od*oh*ow;if(q>=total)return;int ox=q%ow;q/=ow;int oy=q%oh;q/=oh;int oz=q%od;q/=od;int co=q%cout;int n=q/cout;float acc=bias[co];"
    , "  for(int ci=0;ci<cin;++ci)for(int kz=0;kz<kd;++kz)for(int ky=0;ky<kh;++ky)for(int kx=0;kx<kw;++kx){int iz=oz*sd+kz-pd,iy=oy*sh+ky-ph,ix=ox*sw+kx-pw;if(iz>=0&&iz<d&&iy>=0&&iy<h&&ix>=0&&ix<w)acc+=x[(((n*cin+ci)*d+iz)*h+iy)*w+ix]*weights[(((co*cin+ci)*kd+kz)*kh+ky)*kw+kx];}out[int(gid)]=acc;"
    , "}"
    , "inline void jitml_conv3d_backward_data_element(device float *dx,const device float *weights,const device float *dy,const device int *g,uint gid){"
    , "  int cin=g[0],cout=g[1],d=g[2],h=g[3],w=g[4],kd=g[5],kh=g[6],kw=g[7],sd=g[8],sh=g[9],sw=g[10],pd=g[11],ph=g[12],pw=g[13],batch=g[14];int od=(d+2*pd-kd)/sd+1,oh=(h+2*ph-kh)/sh+1,ow=(w+2*pw-kw)/sw+1;"
    , "  int q=int(gid),total=batch*cin*d*h*w;if(q>=total)return;int ix=q%w;q/=w;int iy=q%h;q/=h;int iz=q%d;q/=d;int ci=q%cin;int n=q/cin;float acc=0.0f;"
    , "  for(int co=0;co<cout;++co)for(int kz=0;kz<kd;++kz)for(int ky=0;ky<kh;++ky)for(int kx=0;kx<kw;++kx){int zn=iz+pd-kz,yn=iy+ph-ky,xn=ix+pw-kx;if(zn>=0&&yn>=0&&xn>=0&&zn%sd==0&&yn%sh==0&&xn%sw==0){int oz=zn/sd,oy=yn/sh,ox=xn/sw;if(oz<od&&oy<oh&&ox<ow)acc+=weights[(((co*cin+ci)*kd+kz)*kh+ky)*kw+kx]*dy[(((n*cout+co)*od+oz)*oh+oy)*ow+ox];}}dx[int(gid)]=acc;"
    , "}"
    , "inline void jitml_conv3d_backward_weights_element(device float *dw,device float *db,const device float *x,const device float *dy,const device int *g,uint gid){"
    , "  int cin=g[0],cout=g[1],d=g[2],h=g[3],w=g[4],kd=g[5],kh=g[6],kw=g[7],sd=g[8],sh=g[9],sw=g[10],pd=g[11],ph=g[12],pw=g[13],batch=g[14];int od=(d+2*pd-kd)/sd+1,oh=(h+2*ph-kh)/sh+1,ow=(w+2*pw-kw)/sw+1,wc=cout*cin*kd*kh*kw;int q=int(gid);"
    , "  if(q<wc){int kx=q%kw;q/=kw;int ky=q%kh;q/=kh;int kz=q%kd;q/=kd;int ci=q%cin;int co=q/cin;float acc=0.0f;for(int n=0;n<batch;++n)for(int oz=0;oz<od;++oz)for(int oy=0;oy<oh;++oy)for(int ox=0;ox<ow;++ox){int iz=oz*sd+kz-pd,iy=oy*sh+ky-ph,ix=ox*sw+kx-pw;if(iz>=0&&iz<d&&iy>=0&&iy<h&&ix>=0&&ix<w)acc+=dy[(((n*cout+co)*od+oz)*oh+oy)*ow+ox]*x[(((n*cin+ci)*d+iz)*h+iy)*w+ix];}dw[int(gid)]=acc;return;}"
    , "  int co=q-wc;if(co<cout){float acc=0.0f;for(int n=0;n<batch;++n)for(int oz=0;oz<od;++oz)for(int oy=0;oy<oh;++oy)for(int ox=0;ox<ow;++ox)acc+=dy[(((n*cout+co)*od+oz)*oh+oy)*ow+ox];db[co]=acc;}"
    , "}"
    , ""
    ...
    , "}"
    , ""
    , "inline void jitml_norm_batch_sample(device float *out,device float *dx,device float *dw,device float *db,const device float *x,const device float *gamma,const device float *beta,const device float *dy,const device int *g,const device float *fp,device float *scratch,uint gid){"
    , "  int flavor=g[0],channels=g[1],spatial=g[2],groups=g[3],batch=g[4],len=channels*spatial,n=int(gid);if(n>=batch)return;int ng=flavor==0?channels:(flavor==2?(groups>0?groups:1):1),scratchStride=3*len+6*ng;"
    , "  jitml_norm_train(out+n*len,dx+n*len,dw+n*channels,db+n*channels,x+n*len,gamma+n*channels,beta+n*channels,dy+n*len,g,fp,len,scratch+n*scratchStride);"
    , "}"
    , ""
    ...
    , "  if (gid == 0u) { status[0]=0; }"
    , "  switch(opcode){"
    , "    case 0: if(gid==0u)jitml_dense_train(out,dx,dw,db,x,weights,bias,dy,geom[0],geom[1],geom[2]); break;"
    , "    case 1: if(gid==0u)jitml_conv2d_train(out,dx,dw,db,x,weights,bias,dy,geom); break;"
    , "    case 2: if(gid==0u)jitml_conv3d_train(out,dx,dw,db,x,weights,bias,dy,geom); break;"
    , "    case 10: if(gid==0u)jitml_geglu_train(out,dx,dw,db,x,weights,bias,dy,geom,scratch); break;"
    , "    case 11: if(gid==0u)jitml_norm_train(out,dx,dw,db,x,weights,bias,dy,geom,fparams,xlen,scratch); break;"
    , "    case 12: if(gid==0u)jitml_patch_train(out,dx,dw,db,x,weights,bias,dy,geom,xlen); break;"
    , "    case 13: if(gid==0u)jitml_attention_train(out,dx,dw,db,x,weights,bias,dy,geom,scratch); break;"
    , "    case 14: if(gid==0u)jitml_residual_train(out,dx,dw,db,x,weights,bias,dy,geom,fparams,scratch); break;"
    , "    case 15: if(gid==0u)jitml_pool_train(out,dx,x,dy,geom,xlen); break;"
    , "    case 16: if(gid==0u)jitml_scale_train(out,dx,x,dy,xlen,fparams[0]); break;"
    , "    case 20: jitml_dense_forward_element(out,x,weights,bias,geom[0],geom[1],geom[2],gid); break;"
    , "    case 21: jitml_dense_backward_data_element(dx,weights,dy,geom[0],geom[1],geom[2],gid); break;"
    , "    case 22: jitml_dense_backward_weights_element(dw,db,x,dy,geom[0],geom[1],geom[2],gid); break;"
    , "    case 23: jitml_conv2d_forward_element(out,x,weights,bias,geom,gid); break;"
    , "    case 24: jitml_conv2d_backward_data_element(dx,weights,dy,geom,gid); break;"
    , "    case 25: jitml_conv2d_backward_weights_element(dw,db,x,dy,geom,gid); break;"
    , "    case 26: jitml_conv3d_forward_element(out,x,weights,bias,geom,gid); break;"
    , "    case 27: jitml_conv3d_backward_data_element(dx,weights,dy,geom,gid); break;"
    , "    case 28: jitml_conv3d_backward_weights_element(dw,db,x,dy,geom,gid); break;"
    , "    case 31: jitml_norm_batch_sample(out,dx,dw,db,x,weights,bias,dy,geom,fparams,scratch,gid); break;"
    , "    default: if(gid==0u)status[0]=1; break;"
    , "  }"
    , "}"
    ]
