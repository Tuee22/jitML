...
runCheckCode = runLint LintAll LintCheck
...
runLint target mode =
    case target of
        ...
        LintHaskell -> checkHaskellLintConfig
...
checkFiles mode = do
    ...
    pure (whitespaceFindings <> forbiddenFindings <> generatedFindings)
...
checkHaskellLintConfig :: IO [LintFinding]
checkHaskellLintConfig = do
    ...
    let hlintFindings =
            [ LintFinding ".hlint.yaml" "hlint.config.missing" "missing .hlint.yaml" "create .hlint.yaml with the doctrine-required rules"
            | not hlintExists
            ]
    pure (fourmoluFindings <> hlintFindings <> primitiveFindings)
...
checkRequiredConfig path keys = do
    ...
    if exists
        then do
            ...
            pure
                [ LintFinding path ("fourmolu." <> key) ("missing fourmolu setting: " <> key) "add the setting to fourmolu.yaml"
                | key <- keys
                , not (key `Text.isInfixOf` content)
                ]
        else
            pure
                [ LintFinding path "fourmolu.config.missing" "missing fourmolu.yaml" "create fourmolu.yaml with the doctrine-required settings"
                ]
    ...
    if content == rewritten
        then pure []
            ...
            case mode of
                ...
                LintCheck ->
                    pure
                        [ LintFinding path "files.whitespace" "trailing whitespace or missing final newline" "run `jitml lint files --write`"
                        ]
...
forbiddenPathFindings = do
    ...
    pure [forbiddenFinding path rule | path <- existingForbidden, Just rule <- [matchForbiddenPath path]]
...
generatedPathFindings = do
    drifts <- checkDocs
    pure (fmap docsDriftFinding drifts)
...
checkForbiddenPrimitive path
    | path == "src/JitML/Sub/Stream.hs" = pure []
    | otherwise = do
        content <- Text.IO.readFile path
        pure
            [ LintFinding path key "forbidden subprocess primitive outside typed interpreter" "move subprocess execution through `src/JitML/Sub/Stream.hs`"
            | (key, needle) <- forbiddenPrimitiveNeedles
            , needle `Text.isInfixOf` content
            ]

forbiddenPrimitiveNeedles :: [(Text, Text)]
forbiddenPrimitiveNeedles =
