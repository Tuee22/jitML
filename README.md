...
**Lazy tart spin-up on Apple Silicon.** Bootstrap and host daemon startup never touch tart. On a JIT cache miss the target daemon first uses the typed prerequisite DAG to install or validate the `tart` Homebrew package if needed, then ensures the `jitml-build` VM is reachable. `JitML.Tart.Build` now renders and executes the ordered cache-miss plan: inspect `tart list --source local --format json`, start a stopped VM with `tart run --no-graphics`, poll `tart exec <vm> true`, validate `swift --version` inside the VM, run `swift build` against the generated package, publish `libJitMLMetal.dylib` into `./.build/jit/apple-silicon/<hash>.dylib` atomically (`tmp + rename`), and repoint the stable-named symlink under `./.build/host/apple-silicon/`. Unit tests validate ordered success/failure behavior with a synthetic executor plus the live Tart status parser; Metal FFI loading and host↔cluster RPC remain Phase 7.5 work. The VM stays up for the daemon's lifetime once spun up; an idle timeout (default 30 min, configurable in `LiveConfig`) may bring it down again. Subsequent cache hits skip the spin-up entirely.

Manual VM access is available through the lifecycle and pass-through CLI verbs. `jitml internal vm bootstrap` clones the default Tart source image into `jitml-build` when missing, `up` starts a stopped VM, `status` reports missing/stopped/running, `down` stops a running VM, and `exec` renders a typed `tart exec` command for debugging Swift build failures without dropping into Tart by hand:
...
Per-substrate Kind configs at `./kind/cluster-<substrate>.yaml`. Single control-plane + one worker on Apple Silicon (collocated); identical layout for Linux CPU; Linux CUDA labels the worker `jitml.runtime/gpu=true` so the NVIDIA runtime class binds there. `JitML.Cluster.Kind.kindConfigWithWorkerCount` renders additional workers for live placement validation while leaving the checked-in substrate configs at their default one-worker topology.
...
The orchestrator is a stateless **Deployment** with `replicas: 1` by default and required pod **anti-affinity** at `topologyKey: kubernetes.io/hostname`, which lets the cluster scale to N replicas (one per node) when throughput requires it without ever placing two on the same node. Its rolling update strategy uses `maxSurge: 0` and `maxUnavailable: 1`, so the default single-worker Kind cluster can replace the lone pod without creating an unschedulable surge pod. Live validation on 2026-05-20 scaled the real `jitml-service` Deployment to two replicas on a temporary two-worker Kind cluster and observed one pod per worker; the same date also validated single-worker rollout, service-account kubectl, `/healthz`, `/readyz`, and `/metrics`. The daemon owns no PVC of its own — durable state lives entirely in MinIO and Pulsar — so a StatefulSet (which exists for stable identity + ordered scale-up + per-pod PVCs) would be the wrong shape. Each node maintains its own JIT cache under that node's `./.build/jit/<substrate>/` hostPath, which is fine because JIT artifacts are deterministic functions of `(model-shape, kind, substrate, toolchain)` — the worst case is that the same model gets JITted once per node on first use.
...
Apache Pulsar HA chart: 3× ZooKeeper, 3× BookKeeper, 3× Broker, 3× Proxy, with the admin API routed at `/pulsar/admin`. The Pulsar WebSocket route is `/pulsar/ws`; it rewrites to `/ws` and targets the broker HTTP service (`pulsar-broker:8080`) with `webSocketServiceEnabled=true`. Live validation on 2026-05-19 publishes and consumes through that route with `JitML.Service.PulsarWebSocketSubprocess`; 2026-05-20 validation reconciles the current 26-topic family and publishes/consumes on `training.command.linux-cpu` from `jitml:local`. The image carries a pinned Node.js 22 runtime; the subprocess script uses `globalThis.WebSocket` when available and retains an `undici.WebSocket` fallback for older Node runtimes. The PureScript frontend subscribes to live events through the `jitml-demo` proxy at `/api/ws`.
...
`JitML.Cluster.PulsarBootstrap.pulsarTopics` registers exactly this 26-topic
family during bootstrap: eight command/event topics for each substrate plus the
Apple-only internal RPC pair.
...
runtimeClass validation runs the actual `jitml-service` pod on
`jitml-linux-cuda-worker` and confirms `nvidia-smi -L` reports the RTX 5090
inside that container. Apple Silicon host validation runs the generated
`./.build/conf/host/apple-silicon.dhall` with
`jitml service --consume-once 0`, passes routed MinIO / Harbor / kubectl probes,
and acquires `inference.command.apple-silicon` as `jitml-host`.
