...
| Xcode/Metal | pinned (Xcode 16 pre-installed/pre-licensed in the `jitml-build` tart VM image; never on the host) | `jitml-build` VM image + metal/swift flags in `cabal.project` |
...
- The host daemon executes Metal kernels with direct GPU access but never compiles them: the host keeps only the Metal framework, used solely to load and run the VM-produced `.dylib`, and never runs Xcode. ALL Apple Silicon Swift and Metal shader compilation happens inside the `jitml-build` tart VM — irrespective of VM image size or download cost — because the host cannot run the Metal shader compiler headlessly. Routing every Swift/Metal build through Tart is the only way jitML can truly JIT on Apple Silicon; a hard architectural requirement, not an optimization to trade away. See [Bootstrap scripts](#bootstrap-scripts) and [Built-artifact and JIT-cache discipline](#built-artifact-and-jit-cache-discipline).
...
- `purge` is destructive but **cache-preserving**: cluster down, `rm -rf ./.data/`, tart VM destroyed if it exists (Swift incremental build cache inside the VM is wiped with it). `./.build/` survives — including `./.build/jit/apple-silicon/`, `./.build/runtime/`, `./.build/conf/`, and the Kind metadata needed for subsequent `docker compose run --rm jitml jitml <command>` calls. A subsequent bootstrap or inference command can resolve from cache without re-JITting any model already compiled. The cache is the payoff: it lets a previously compiled kernel load without re-running the VM build, so tart need only be spun up when a *new* model shape or *new* kind (training vs inference) appears. Tart remains mandatory for every genuine compile, though — any new `(model-shape, kind, substrate, toolchain)` tuple MUST be built in the VM, because that is the only headless path to Xcode's `metal` compiler. The cache reduces redundant rebuilds; it never makes host-side Metal compilation an option.
...
- **Metal codegen:** the VM-pinned Xcode 16 toolchain (inside the `jitml-build` tart VM, never on the host), `-O2`, no fast-math.
