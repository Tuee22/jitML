...
🔄 **Active** (reopened 2026-06-04 for Sprint `8.9`). The original
...
The current reopen replaces ROM-dependent default/canonical RL examples with
the copyright-free repo-owned `KeyDoorGrid-v0` environment. The active
...
convergence assertions are owned by Phase `13`; Phase `8` currently owns the
Sprint `8.9` KeyDoorGrid replacement work.
...
Sprint `8.9` reopens this phase to make the default visual RL demo
copyright-free by adding `KeyDoorGrid-v0` and moving examples away from
`atari-subset`.
...
`src/JitML/App.hs`, `test/unit/Main.hs`, `test/rl-canonicals/Main.hs`,
`experiments/`
**Docs to update**: `README.md`,
`documents/engineering/training_workloads.md`,
`documents/engineering/unit_testing_policy.md`,
...
- Implement `KeyDoorGrid-v0` in the Haskell simulator/environment catalog,
  including deterministic layout generation, action masks, render frames, and
  canonical tests.
- Move default RL examples and user-facing demo prose away from `atari-subset`.
- Keep the development ledger row open until the required tests and
  `check-code` pass.
