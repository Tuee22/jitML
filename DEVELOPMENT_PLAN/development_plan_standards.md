...
├── phase-241-literal-architectures-dense-mlp-lenet.md
├── phase-242-literal-architectures-resnet-family.md
├── phase-243-literal-architectures-vision-transformer.md
├── phase-244-convergence-and-evidence.md
├── phase-245-completedtraining-sl-manifests.md
├── phase-246-real-environments.md
├── phase-247-distinct-algorithms.md
├── phase-248-per-row-convergence-and-evidence.md
├── phase-249-typed-rl-cohort-and-action-domain-compatibility.md
├── phase-250-trainingplan-evaluationplan-compiler-and-trainer-migration.md
├── phase-251-typed-measured-counters-and-evidence-separation.md
├── phase-252-per-game-self-play.md
├── phase-253-arena-convergence-evidence.md
├── phase-254-train-and-publish-artifact-selectors.md
├── phase-255-row-specific-renderers.md
├── phase-256-browser-fail-closed.md
├── phase-257-row-keyed-integration-matrix.md
├── phase-258-row-complete-playwright.md
├── phase-259-linux-cpu-report-card.md
├── phase-260-contract-driven-live-execution-integration-journal.md
├── phase-261-contract-driven-live-execution-browser-and-playwright.md
├── phase-262-contract-driven-live-execution-fragment-issuance.md
├── phase-263-real-cudnn-cublas-kernels.md
├── phase-264-cuda-row-device-evidence.md
├── phase-265-cuda-integration-e2e-and-attestation.md
├── phase-266-gpu-performance-and-persistent-device-buffers.md
├── phase-267-contract-driven-cuda-lane-revalidation.md
├── phase-268-real-metal-kernels.md
├── phase-269-metal-row-device-evidence.md
├── phase-270-apple-integration-e2e-and-attestation.md
├── phase-271-contract-driven-apple-lane-revalidation.md
├── phase-272-attestation-join.md
├── phase-273-no-caveat-closure-guard.md
├── phase-274-journal-derived-product-aggregation.md
├── phase-275-negative-control-suite.md
├── phase-276-external-bars-no-self-referential-gate-lint-and-exact-served.md
├── phase-277-measured-declared-type-split-behavioral-scaffold-lint.md
├── phase-278-runcontract-negative-controls-request-and-event-fixtures.md
├── phase-279-runcontract-negative-controls-journal-fixtures-and-reducer-p.md
├── phase-280-runcontract-negative-controls-lifecycle-and-per-row-registra.md
├── phase-281-per-model-measured-convergence.md
├── phase-282-inference-performance-determinism.md
├── phase-283-contract-driven-per-model-evidence.md
├── phase-284-evidence-derived-closure-guard.md
├── phase-285-standing-adversarial-audit-thin-plan.md
├── phase-286-journal-derived-status-registry.md
├── phase-287-evidence-typed-report-measurements.md
```
...
The strict forward chain runs `1 → … → 287` in the single-session numbering (the 2026-07-26 IR-single-owner `+4` renumber inserted Phases `236`–`239` and shifted the former tail `236`–`283` to `240`–`287`). The current open suffix (the phases not yet Done) is `234 → 235 → 236 → 237 → 238 → 239 → 240 → 241 → 242 → 243 → 244 → 245 → 249 → 250 → 251 → 260 → 261 → 262 → 267 → 271 → 274 → 276 → 278 → 279 → 280 → 283 → 286 → 287`. Sprint `23.1` is Done — its correct reverse-mode autodiff node
...
handled by the IR redesign in Phases `235`–`239`). Phase `234` (oneDNN layer
kernels for training) is the **reopened** `Active` frontier (2026-07-26): its
per-example kernels are Done and validated on the `linux-cpu` container lane
(jitml-backends 24/24, jitml-unit 766/766, check-code ok), and its new obligation
is the batched IR kernels. Phase `235` is redefined as the one self-describing
checkpoint envelope, and `236`–`239` are the inserted
admission/serving/training/construction phases; every later phase in that chain is
