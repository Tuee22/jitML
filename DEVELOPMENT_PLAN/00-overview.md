...
runners. Sprints `1.1`, `1.2`, `1.3`, and `1.5` through `1.9` remain closed on
their owned surfaces, and Phase `2` Sprints `2.1` through `2.3` remain closed on
their bootstrap/prerequisite/cache surfaces. The overall handoff remains
incomplete until Sprint `1.4` and the remaining Phase `2` sprints close.

| Surface | Current Repo State | Intended End State |
|---------|--------------------|--------------------|
| Repository layout | Sprints `1.1` through `2.3` have landed `jitml.cabal`, `cabal.project`, `app/Main.hs`, `app/Demo.hs`, `src/JitML/App.hs`, `src/JitML/AppError/{AppError,Render}.hs`, `src/JitML/Cache/{Key,Layout,Manifest,Symlink}.hs`, `src/JitML/CLI/{Spec,Parser,Tree,Json,Help,Output}.hs`, `src/JitML/Docs/{Check,Generate,Render}.hs`, `src/JitML/Env/{Env,Build}.hs`, `src/JitML/Generated/{Registry,Paths}.hs`, `src/JitML/Lint/{Chart,ForbiddenPaths,Stack}.hs`, `src/JitML/Plan/{Plan,Apply,Render}.hs`, `src/JitML/Sub/{Subprocess,Render,Stream}.hs`, `src/JitML/Prerequisite/{Registry,Reconcile,Plan,Nodes/*}.hs`, `bootstrap/{_lib,apple-silicon,linux-cpu,linux-cuda}.sh`, `fourmolu.yaml`, `.hlint.yaml`, `documents/cli/commands.md`, `share/man/man1/jitml.1`, shell completion scripts, `test/golden/{cache,cli,prerequisite}/`, `test/sentinel/Main.hs`, `test/unit/Main.hs`, `test/integration/Main.hs`, `test/haskell-style/Main.hs`, `.gitignore`, and `.dockerignore`; there is no `chart/`, `kind/`, `docker/`, `web/`, `infra/`, `proto/`, `codegen-cuda/`, `codegen-metal/`, `codegen-onednn/`, or `experiments/` yet | Full library-first Haskell layout per [../README.md → Repository layout (target)](../README.md#repository-layout-target) |
| Build artefacts | `cabal build all` produces the Phase `1.9` `jitml` and `jitml-demo` binaries under `dist-newstyle/`; `bootstrap/apple-silicon.sh build` produces the host-native handoff binary at `./.build/jitml`; the typed JIT cache key/layout/manifest/symlink layer is implemented, but no real codegen artefacts are produced until later codegen sprints | `cabal build all`-produced `jitml` and `jitml-demo` binaries, plus per-substrate JIT-cache artefacts under `./.build/jit/<substrate>/` |
| CLI surface | The full command family is registered and parseable from `CommandSpec`; `jitml bootstrap`, `jitml doctor`, `jitml doctor --remediate`, `jitml commands`, focused help, `jitml docs check/generate`, `jitml lint`, `jitml check-code`, `--dry-run`, `--plan-file`, the `Env` record, default/env/CLI directory resolution, global `--format` / `--color` flags, JSON/plain command output, structured `AppError` rendering, `internal list-prereqs`, the scaffolded `internal materialize-substrate` substrate validator, and the stage-0 script handoff are implemented. The current lint stack enforces config presence, whitespace normalization, forbidden paths, generated-doc drift, and forbidden subprocess primitives; external `fourmolu`, `hlint`, `cabal format`, and warning-clean build execution remains open in Sprint `1.4`. Real substrate file materialization and mutating command bodies outside docs/lint remain blocked on their owning later sprints; the `jitml bootstrap` full cluster apply body continues in later Phase `3` cluster work | The complete command family parses and runs against three substrates: `doctor`, `cluster {up,down,status,reset}`, `service`, `train`, `eval`, `tune`, `rl {train,eval,rollout}`, `verify {same-run,cross-backend,replay}`, `inspect {list,show,replay,trial,frontier}`, `bench {train,inference,env}`, `inference run`, `test`, `lint`, `docs`, `check-code`, `build`, `kubectl`, `internal {materialize-substrate,list-prereqs,gc,vm,cache}`, `commands`, `help`, plus the `jitml-demo` HTTP server |
| Test stanzas | Ten Cabal stanzas are declared and pass under `cabal test all`; `jitml-unit` owns parser/registry tests, `jitml-haskell-style` owns the current in-repo lint stack, and the other phase-owned bodies remain blocked until their owning sprint lands | Ten Cabal stanzas: `jitml-unit`, `jitml-integration`, `jitml-sl-canonicals`, `jitml-rl-canonicals`, `jitml-hyperparameter`, `jitml-cross-backend`, `jitml-daemon-lifecycle`, `jitml-e2e`, `jitml-haskell-style`, `jitml-purescript-style` |
