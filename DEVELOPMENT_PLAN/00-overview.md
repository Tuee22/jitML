...
the repository. Phase `8` is reopened for Sprint `8.9`, which adds the repo-owned
`KeyDoorGrid-v0` environment so default RL demos and required canonical
examples need no copyrighted ROM material. Phase `9` is reopened and blocked on
that work for Sprint `9.8`, which retargets the RL algorithm/convergence matrix.
...
- **Test stanzas, lint matrix, cross-cluster parity.** Eight Cabal
  ...
  The apple-involving cross-substrate parity path is validated by
  Sprint `15.1`. The seven doctrine
...
28. Target JIT compiler inputs are generated by the Haskell `jitml` binary on a
    ...
    `.cpp`, Metal / Swift package source files, native adapter shims, or
    per-substrate JIT build `.sh` scripts as build inputs. The Haskell renderers
    emit those files into
    ...
    Checked-in native/foreign source exceptions are not allowed; adapter code
    belongs in the same Haskell-generated materialization path or outside the
    repository.
...
| 15 | Phase 13, Phase 14, Phase 1 Sprint `1.10`, Phase 8 Sprint `8.9`, Phase 9 Sprint `9.8` | Cross-substrate parity cohort, populated live `jitml test all` report card, and empty legacy ledgers require live outputs from both Phase `13` (Linux CUDA) and Phase `14` (Apple Silicon); the 2026-06-03 Apple weighted bundle, Linux/Apple report-bundle comparison, `--live` code surface, container `jitml check-code` gate, the 2026-06-04 fresh Apple live full-aggregate `jitml test all --live` report-card pass, the demo-placeholder cleanup, the scoped `allow-newer` removal, and optional ALE support are present, while the `KeyDoorGrid-v0` development row and dependency source-pin/vendor helper keep Phase `15` blocked |
...
legacy-deletion ledger row. Phase `8` is `🔄 Active` for Sprint `8.9`
(`KeyDoorGrid-v0` copyright-free visual RL demo). Phase `9` is `⏸️ Blocked` on
that environment work for Sprint `9.8`. Phase `15` (cross-substrate parity +
final handoff) is `⏸️ Blocked` until both the development row and the
source-pin/vendor helper move to Completed. Phases `2`, `3`, `4`, and `5`
...
Sprint `8.9` now owns the `KeyDoorGrid-v0` copyright-free default demo
replacement. Phase `9` Sprint `9.8` is blocked on that environment before it
can retarget the required RL matrix.
