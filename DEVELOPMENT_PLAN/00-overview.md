...
The 2026-07-15 retained-cluster audit reopened Sprints `2.9` and `3.7`; both
...
code `3` for an exact retained no-op. Sprint `12.16` then re-closed on immutable
descriptor
`sha256:6e0d57971bf8e6a7c996530a4b434a575237a570c745710f2a150a501da42aa0`
after its six-command canonical `linux-cpu` block, docs, code-quality, and
resource-verification gates passed. The 2026-07-18 checkpoint/runtime audit then
found that the supervised publisher's apparently eligible artifacts were
...
The current strict forward chain is `234 → 235 → 236 → 237 → 238 → 239 → 240 → 241 → 242 → 243 → 244 → 245 → 249 → 250 → 251 → 260 → 261 → 262 → 267 → 271 → 274 → 276 → 278 → 279 → 280 → 283 → 286 → 287`. Phase `234` (oneDNN layer
kernels) is the **reopened** sole `Active` frontier — its per-example kernels are
Done and validated on `linux-cpu` (jitml-backends 24/24, jitml-unit 766/766,
check-code ok), and its new obligation is the batched IR kernels. Phase `235` is
redefined as the one self-describing checkpoint envelope; `236`–`239` are the
inserted admission/serving/training/construction phases; the former SL phases are
redefined and the tail is renumbered `+4` to `240`–`287`. Every phase after `234`
is Blocked by its immediate predecessor. Phase `10` validated its reopened work on `linux-cpu` only.
...
writer reads the current ETag, requires its pointer CAS to return
`PointerWritten` for the exact manifest, and publishes completion only after
that result. An absent optional legacy weight projection does not gate a generic
below-bar successful miss; a supplied projection remains subject to exact
equality checks.
...
publisher/executor command, kind-indexed plan, substrate, and `PlanId`; report
admission then requires the opaque completed exact-plan join rather than a raw
handle or registry declaration.
...
Its workflow cells remain the public-CLI `Subprocess` / `ProcessOutcome` matrix;
ProductRow model cells are a separate internal exact-executor matrix derived
from the opaque `projectProductRows` batch and invoke
`jitml internal train-and-publish-product-rows --<substrate> --row <rowId>`.
Neither matrix enumeration nor a successful projection can by itself mint a
completed ProductRow report cell.
...
- **Checkpointing and inference-only read path.** The retained V1/legacy surface
  ...
  checkpoint store, split `writeCandidateCheckpointSnapshot{,WithMinIO}` and
  `writeCompletedCheckpointSnapshot{,WithMinIO}` operations with opaque
  `StoredCandidateCheckpoint` / `StoredCompletedCheckpoint` results, exact
  absent-or-byte-identical immutable writes, and exact pointer CAS. Store's
  `admitLatestCheckpoint` and `admitCheckpointAt` return opaque exact persisted
  graphs; `requireAdmittedCompletedCheckpoint` is the only completion admission
  used by `loadInferenceCheckpointWith` and
  `loadInferenceCheckpointWithWeights` before decoded `.jmw1` weights reach the
  selected runner. Admission checks manifest experiment/content addresses,
  tensor payload addresses, exact JMW1 bytes, tensor shapes, and virtual slices,
  ...
  routes use an injected checkpoint runtime handler that calls
...
- **PureScript frontend and demo.** Minimal PureScript entrypoint,
  ...
  modules, `web/test/Main.purs` smoke suite, the live Playwright matrix at
  `playwright/jitml-demo.spec.ts`, `src/JitML/Web/Server.hs` HTTP
  ...
  The generated PureScript contract exposes typed
  ...
  MNIST/generic/CIFAR/checkpoint-compare/Connect 4. The current panels use those
  surfaces rather than marker/default parsers and provide training/RL/tune
  command-envelope controls, training metadata, RL replay scrub summaries, and
  multi-game adversarial boards, and render non-placeholder loss/policy/MCTS/tuning
  ...
  current no-caveat product claim is withdrawn until the Sprint `10.6` → `34.3`
...
- **Long-Running Daemons in the Same Binary** — `jitml service` has refined
  ...
  kinds. Sprint `12.16` completed the opted-in target with the operational
  ...
  evidence. Its immutable-image canonical validation is complete.
...
22. `jitml service` is a long-running daemon parameterised by refined Dhall
    ...
    renderer is retained, and the operational sink reads the dynamic filter for
    every stderr emission.
23. The daemon's Pulsar consumer is at-least-once. Idempotency is the consumer's
    responsibility: strict decode derives semantic identity from the resolved
    plan, command kind, and logical key rather than payload bytes or a
    client-supplied ID. The current local subscription
    ...
    policy is a typed value with named strategies, and every newly scheduled
    operation reads the active production policy.
...
| 237 | [Literal Architectures - Dense, MLP, LeNet](phase-241-literal-architectures-dense-mlp-lenet.md) | ⏸️ Blocked | 24.1 |
| 238 | [Literal Architectures - ResNet Family](phase-242-literal-architectures-resnet-family.md) | ⏸️ Blocked | 24.2 |
| 239 | [Literal Architectures - Vision Transformer](phase-243-literal-architectures-vision-transformer.md) | ⏸️ Blocked | 24.3 |
| 240 | [Convergence and Evidence](phase-244-convergence-and-evidence.md) | ⏸️ Blocked | 24.4 |
| 241 | [CompletedTraining SL Manifests](phase-245-completedtraining-sl-manifests.md) | ⏸️ Blocked | 24.5 |
| 242 | [Real Environments](phase-246-real-environments.md) | ✅ Done | 25.1 |
| 243 | [Distinct Algorithms](phase-247-distinct-algorithms.md) | ✅ Done | 25.2 |
| 244 | [Per-Row Convergence and Evidence](phase-248-per-row-convergence-and-evidence.md) | ✅ Done | 25.3 |
| 245 | [Typed RL Cohort and Action-Domain Compatibility](phase-249-typed-rl-cohort-and-action-domain-compatibility.md) | ⏸️ Blocked | 25.4 |
| 246 | [TrainingPlan/EvaluationPlan Compiler and Trainer Migration](phase-250-trainingplan-evaluationplan-compiler-and-trainer-migration.md) | ⏸️ Blocked | 25.5 |
| 247 | [Typed Measured Counters and Evidence Separation](phase-251-typed-measured-counters-and-evidence-separation.md) | ⏸️ Blocked | 25.6 |
| 248 | [Per-Game Self-Play](phase-252-per-game-self-play.md) | ✅ Done | 26.1 |
| 249 | [Arena Convergence + Evidence](phase-253-arena-convergence-evidence.md) | ✅ Done | 26.2 |
| 250 | [Train-and-Publish + Artifact Selectors](phase-254-train-and-publish-artifact-selectors.md) | ✅ Done | 27.1 |
| 251 | [Row-Specific Renderers](phase-255-row-specific-renderers.md) | ✅ Done | 27.2 |
| 252 | [Browser Fail-Closed](phase-256-browser-fail-closed.md) | ✅ Done | 27.3 |
| 253 | [Row-Keyed Integration Matrix](phase-257-row-keyed-integration-matrix.md) | ✅ Done | 28.1 |
| 254 | [Row-Complete Playwright](phase-258-row-complete-playwright.md) | ✅ Done | 28.2 |
| 255 | [linux-cpu Report Card](phase-259-linux-cpu-report-card.md) | ✅ Done | 28.3 |
| 256 | [Contract-Driven Live Execution - Integration Journal](phase-260-contract-driven-live-execution-integration-journal.md) | ⏸️ Blocked | 28.4 |
| 257 | [Contract-Driven Live Execution - Browser and Playwright](phase-261-contract-driven-live-execution-browser-and-playwright.md) | ⏸️ Blocked | 28.5 |
| 258 | [Contract-Driven Live Execution - Fragment Issuance](phase-262-contract-driven-live-execution-fragment-issuance.md) | ⏸️ Blocked | 28.6 |
| 259 | [Real cuDNN/cuBLAS Kernels](phase-263-real-cudnn-cublas-kernels.md) | ✅ Done | 29.1 |
| 260 | [CUDA Row Device Evidence](phase-264-cuda-row-device-evidence.md) | ✅ Done | 29.2 |
| 261 | [CUDA Integration, E2E, and Attestation](phase-265-cuda-integration-e2e-and-attestation.md) | ✅ Done | 29.3 |
| 262 | [GPU Performance and Persistent Device Buffers](phase-266-gpu-performance-and-persistent-device-buffers.md) | ✅ Done | 29.4 |
| 263 | [Contract-Driven CUDA Lane Revalidation](phase-267-contract-driven-cuda-lane-revalidation.md) | ⏸️ Blocked | 29.5 |
| 264 | [Real Metal Kernels](phase-268-real-metal-kernels.md) | ✅ Done | 30.1 |
| 265 | [Metal Row Device Evidence](phase-269-metal-row-device-evidence.md) | ✅ Done | 30.2 |
| 266 | [Apple Integration, E2E, and Attestation](phase-270-apple-integration-e2e-and-attestation.md) | ✅ Done | 30.3 |
| 267 | [Contract-Driven Apple Lane Revalidation](phase-271-contract-driven-apple-lane-revalidation.md) | ⏸️ Blocked | 30.4 |
| 268 | [Attestation Join](phase-272-attestation-join.md) | ✅ Done | 31.1 |
| 269 | [No-Caveat Closure Guard](phase-273-no-caveat-closure-guard.md) | ✅ Done | 31.2 |
| 270 | [Journal-Derived Product Aggregation](phase-274-journal-derived-product-aggregation.md) | ⏸️ Blocked | 31.3 |
| 271 | [Negative-Control Suite](phase-275-negative-control-suite.md) | ✅ Done | 32.1 |
| 272 | [External Bars, No-Self-Referential-Gate Lint, and Exact Served-Byte Provenance](phase-276-external-bars-no-self-referential-gate-lint-and-exact-served.md) | ⏸️ Blocked | 32.2 |
| 273 | [Measured/Declared Type Split & Behavioral Scaffold Lint](phase-277-measured-declared-type-split-behavioral-scaffold-lint.md) | ✅ Done | 32.3 |
| 274 | [RunContract Negative Controls - Request and Event Fixtures](phase-278-runcontract-negative-controls-request-and-event-fixtures.md) | ⏸️ Blocked | 32.4 |
| 275 | [RunContract Negative Controls - Journal Fixtures and Reducer Properties](phase-279-runcontract-negative-controls-journal-fixtures-and-reducer-p.md) | ⏸️ Blocked | 32.5 |
| 276 | [RunContract Negative Controls - Lifecycle and Per-Row Registration](phase-280-runcontract-negative-controls-lifecycle-and-per-row-registra.md) | ⏸️ Blocked | 32.6 |
| 277 | [Per-Model Measured Convergence](phase-281-per-model-measured-convergence.md) | ✅ Done | 33.1 |
| 278 | [Inference-Performance & Determinism](phase-282-inference-performance-determinism.md) | ✅ Done | 33.2 |
| 279 | [Contract-Driven Per-Model Evidence](phase-283-contract-driven-per-model-evidence.md) | ⏸️ Blocked | 33.3 |
| 280 | [Evidence-Derived Closure Guard](phase-284-evidence-derived-closure-guard.md) | ✅ Done | 34.1 |
| 281 | [Standing Adversarial Audit & Thin Plan](phase-285-standing-adversarial-audit-thin-plan.md) | ✅ Done | 34.2 |
| 282 | [Journal-Derived Status Registry](phase-286-journal-derived-status-registry.md) | ⏸️ Blocked | 34.3 |
| 283 | [Evidence-Typed Report Measurements](phase-287-evidence-typed-report-measurements.md) | ⏸️ Blocked | 34.4 |
...
suffix is:
`234 → 235 → 236 → 237 → 238 → 239 → 240 → 241 → 242 → 243 → 244 → 245 → 249 → 250 → 251 → 260 → 261 → 262 → 267 → 271 → 274 → 276 → 278 → 279 → 280 → 283 → 286 → 287`.
...
`234` is the reopened `Active` frontier; every later phase in the chain is Blocked by its immediate predecessor (the 2026-07-26 IR-single-owner `+4` renumber inserted Phases `236`–`239` and shifted the tail to `240`–`287`). Prior Sprint
...
passthrough), `12.10` (realign `jitml-backends` to within-substrate cases,
...
compose service —
`docker compose run --rm jitml-cuda cabal test -fcuda jitml-backends
...
`jitml-backends -fcuda` 38 / 38 (incl. `CrossSubstrate`), a fresh
