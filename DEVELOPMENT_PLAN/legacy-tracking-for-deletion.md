...
after the linux-cuda validation pair passed. Phase `10` has its Sprint `10.6`
code surface in place but is Blocked on live Linux CPU/CUDA and Apple Silicon
integration validation; Phases `11`–`12` are Active again, Phases `13`–`15` are
Blocked again, and Phases `16`–`18` are added for the full runtime,
...
**Current state (2026-06-14):** Pending Removal is open again for temporary
...
|---|---|---|---|
| Demo marker parsers and default display values | `web/src/Panels/{Mnist,Cifar,Training,Tune,Connect4}.purs` | Text-marker parsers and catch-all `data:` handlers can fabricate `topClass`, `topK`, zero training/tune frames, and empty visit distributions instead of consuming generated typed payloads. The RL `data:` placeholder parser was removed in Sprint `8.12`; this row stays open for the remaining panels. | Sprint `11.9` / Sprint `17.1` |
| Placeholder visualization renderers | `web/src/Panels/Training.purs`, `web/src/Panels/Rl.purs`, `web/src/Panels/Tune.purs`, `web/src/Panels/Connect4.purs` | Canvas placeholders, text-only tables, and non-replay board updates stand in for charts, RL animations, tuning frontiers, adversarial-game rendering, and interactive replay. | Sprint `11.9` / Sprint `17.1` |
| Catalog rollout compatibility helper | `src/JitML/RL/Algorithms/Common.hs` (`trajectoryRollout`) and registered `moduleRolloutGenerator` fields | The helper now steps real environment dynamics and is no longer a reward-derived projection, but it remains a deterministic catalog compatibility surface distinct from the checkpoint-backed train/eval/rollout product matrix. | Sprint `16.1` |
| Browser-contract payload gap | `src/JitML/Web/Contracts.hs`, `web/src/Generated/Contracts.purs` | The generated browser contract now includes RL animation/replay payload records, but still lacks the full typed command/event/result/control ADTs needed to remove the remaining panel-side marker parsing. | Sprint `11.9` / Sprint `12.13` |
| Dense-only SL compatibility helper | `src/JitML/SL/Canonicals.hs` (`denseMlpCohort`), legacy Dense-only classifier entrypoints in `src/JitML/SL/Classifier.hs` | `trainableCanonicalCohort` and `JitML.SL.Architecture` now drive the product train-step surface for all canonical rows. The old Dense cohort/helper entrypoints remain only for compatibility tests and must disappear once all-row live convergence, checkpointing, inference, and docs no longer name them. | Sprint `16.1` |
...
|------|------------|-------|
| Demo-only inline model responses | Sprint `10.6` (2026-06-15) | Removed `renderInferenceResponse`, `renderImageResponse`, and `renderConnect4Response` from `src/JitML/Web/Server.hs` along with the inline `PolicyValueNet` / initial-board imports. `/api/inference`, `/api/images`, and `/api/connect4/move` now fail closed with `503 checkpoint-required` until Phase `17` supplies checkpoint-backed browser requests. Validation: `jitml-unit --linux-cpu` passed 197 / 197, non-live `jitml-integration` passed 51 / 51, and `jitml-daemon-lifecycle --linux-cpu` passed 34 / 34. |
...
| Demo endpoint `/api/images` placeholder and live-cluster round-trip residue | Sprint `11.8` / Phase `13` (2026-06-11) | At Sprint `11.8` closure, `src/JitML/Web/Server.hs` routed `/api/images` to `renderImageResponse`, which returned the policy-network top-k vector rather than an upload acknowledgement. Sprint `10.6` later removed that inline response function and made the route fail closed until checkpoint-backed browser requests land. After `docker compose build jitml` passed `check-code: ok` and rebuilt the PureScript bundle, `jitml:local` and `jitml-demo:local` were loaded into the `linux-cuda` Kind cluster, `jitml-service` / `jitml-demo` were rollout-restarted, and the Playwright Docker image passed **9 / 9** against the published edge. |
