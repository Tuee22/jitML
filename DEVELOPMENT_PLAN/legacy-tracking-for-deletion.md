...
for its local framework/runtime surface; Phases `9`–`12` are Active again,
Phases `13`–`15` are Blocked again, and Phases `16`–`18` are added for the full
runtime, browser/Playwright, and final handoff closure. The primary
...
| Demo-only inline model responses | `src/JitML/Web/Server.hs` (`renderInferenceResponse`, `renderImageResponse`, `renderConnect4Response`) | Demo responses are produced from inline demo networks or initial-board helpers instead of the checkpoint selected by the workflow. Final product interaction must route through checkpoint-backed inference/game state. | Sprint `10.6` / Sprint `11.9` |
