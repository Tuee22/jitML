...
**Current state (2026-07-05): Pending Removal reopened by the realness audit.**
Exit-Definition item `18` (empty ledger) is **not met**. The audit found the
Phase `19`–`31` closure evidence was substantially fabricated while the anti-fake
gates were tautological or unenforced; the fabrication rows below are enqueued for
removal, and two previously-`Completed` rows are reopened (see the `Completed`
table). The pre-existing accelerator-kernel scaffold rows remain open. Each row is
removed when its owning reopened sprint lands and its lane validates against the
Phase `32` negative-control suite.
...
| **Expert-controller RL reward**: `canonicalDiscreteEvaluation` / `canonicalContinuousEvaluation` / `*ExpertAction` in `src/JitML/App.hs` supply reported episode returns from a hardcoded controller via `fromMaybe (trained-policy eval) (canonicalEval …)`, discarding the trained policy | Sprint `25.x` | Deleted; reported reward comes only from the trained-policy evaluator, verified by a negative control (an expert-controller reward trace is rejected) and by "deleting the controller changes nothing". |
| **HER constant reward**: `herEpisodes` returns `replicate evalEpisodes (1.0, herNumBits)` in `src/JitML/App.hs` | Sprint `25.x` | Replaced by a real goal-conditioned rollout of the trained HER policy. |
| **Decorative `archLayerGraph` + identity/dense exotic layer kinds**: `src/JitML/SL/Architecture.hs` trains `archLayers` (residual-MLP over flattened pixels) while `archLayerGraph` is never trained; `src/JitML/Numerics/LayerGraph.hs` computes Conv2D/attention/norm/Bottleneck as a plain dense matmul or identity no-op | Sprints `23.x` / `24.x` | The trained topology is genuinely convolutional/attentional; the parallel label graph is removed; verified by a differential test (conv output ≠ dense output of the same shape). |
| **RL algorithm stand-ins**: TRPO (no conjugate-gradient trust region), RecurrentPPO (no recurrent cell/state), SAC (fixed `α`, deterministic actor, no entropy term), TQC (scalar critics, `drop=0`), CrossQ (hardcoded identity "batch-renorm") in `src/JitML/RL/Algorithms/PpoTrainer.hs` and `ContinuousTrainer.hs` | Sprint `25.x` | Each algorithm's defining mechanism is implemented and wired into the trainer (the real `*Loss` modules are used); verified by differential tests (e.g. `TQC(drop>0) ≠ SAC`, SAC actor entropy > 0). |
| **Decorative RL/tuning hyperparameters**: declared-but-unconsumed knobs (`Trpo.hs` cg-/backtrack-, `Sac.hs` `alpha=auto`, A2C RMSProp/lr, TQC drop-atoms) | Sprint `25.x` | Wired to the trainer or removed. |
| **Non-adaptive tuning search**: `sampledClassifierConfig` in `src/JitML/Tune/Catalog.hs` is a grid indexed by trial number (sampler kind only changes a seed); the pruner and ASHA scheduler are never applied | Sprint `25.x` | Real TPE (Parzen) sampler conditioned on prior objectives, real ASHA rung promotion, real MedianPruner. |
| **AlphaZero product-path truncation**: `trainAndPublishAlphaZeroProductRow` in `src/JitML/App.hs` runs one generation with `maxPlies=4`, so the arena win-rate is a `0.5`-draw artifact; othello/hex/gomoku demo checkpoints (`policyValuePanelDemo`) are untrained random-init networks | Sprint `26.x` | Full `maxPlies`, the declared multi-generation budget, a strict win-margin arena threshold, and trained per-game demo checkpoints. |
| **`FutureOwner` scaffold-lint exemption**: `src/JitML/Lint/ProductTruth.hs` registers `seeded-demo-weights` as `FutureOwner` so it is never scanned; the lint is a name-denylist of prior fossils | Sprint `20.x` | Behavioral fake detector (does a product function's output depend on the trained weights?); the `FutureOwner` exemption is deleted. |
| **Tautological convergence bar**: `convergenceObservationsForMetrics` in `src/JitML/App.hs` builds each bar with `mkConvergenceBar name Maximise value 0.0`, so `threshold = value` and `evaluateConvergence` tests `value ≥ value` | Sprints `19.x` / `21.x` | Bars are frozen external constants (`src/JitML/Product/ExternalBars.hs`); a lint bans any threshold derived from the measured value; `coPassed` is re-derived at decode. |
| **All-zeros weight-movement evidence**: `checkpointTrainingEvidenceWithDatasetSha` in `src/JitML/App.hs` hashes an all-zeros vector as the "initial" weights, so any nonzero final weights satisfy movement | Sprint `21.x` | The initial hash is the real random-init weights; `updateCount` equals the real optimizer-step count. |
...
| Fabricated `coPassed = True` / `coThreshold = Nothing` completion witness in `JitML.Training.Budget.completedTrainingFromMetrics` | Sprint `21.1` (2026-07-02 ledger move) | ⚠️ **REOPENED 2026-07-05 — this closure was incorrect.** The witness no longer hardcodes `coPassed`, but the tautology was **relocated, not removed**: the checkpoint writer now builds the bar with `mkConvergenceBar name Maximise value 0.0`, so `threshold = value` and `evaluateConvergence` tests `value ≥ value` — still always True. Reopened as the "Tautological convergence bar" row in Pending Removal above (Sprints `19.x`/`21.x`). |
...
| Seeded demo checkpoint catalog and seeded experiment list | Sprints `10.10` / `14.4` (2026-06-26) | ⚠️ **REOPENED 2026-07-05 — this closure was incorrect.** `policyValuePanelDemo` (othello/hex/gomoku) writes an **untrained** random-init `initPolicyValueNet` through the completed-witness path, so these manifests **do** decode as `InferenceEligible`; the only thing stopping them serving is an ad-hoc `"-demo-weights" isInfixOf` string filter in `src/JitML/Web/Server.hs`, not the type-state. Reopened as the "AlphaZero product-path truncation" row in Pending Removal above (Sprint `26.x`); the type-state fix is owned by reopened Phase `21`. |
