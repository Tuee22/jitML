...
| Hand-written catalog/experiment Dhall schema files | `dhall/numerics/Schema.dhall`, `dhall/rl/Schema.dhall`, `dhall/**/Schema.dhall`, `experiments/*.dhall` types | Sprint `5.12` landed reflected schema for the **daemon config** surfaces (`BootConfig`/`LiveConfig`/`RunConfig`) via `JitML.Service.DhallSchema` — emitted off the live `FromDhall` decoders (`Dhall.expected`), exposed by `jitml internal dhall-schema`, and parity-tested in `jitml-unit` against the checked-in files. The remaining hand-written catalog (`dhall/numerics`, `dhall/rl`) and `experiments/*.dhall` schemas (today validated by separate catalog-mirror tests) still need reflected emission for full anti-drift. | Phase `5` (common-shape reflected-schema reopen) |
