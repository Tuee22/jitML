...
| Deterministic MCTS prior stub | `src/JitML/RL/AlphaZero/Mcts.hs` | `priorFor` stands in for the real policy/value network call so MCTS and transposition-table tests are deterministic before AlphaZero uses JIT-backed network inference. Sprint 13.9 (2026-05-27) added `JitML.RL.AlphaZero.EnginePrior.buildLinuxCpuPriorOracle` that drives the prior from a real JIT-compiled Dense2D kernel, and `runSelfPlayWithPrior` accepts that closure — the stub remains the `defaultPriorOracle` only until a full policy/value network codegen + checkpoint surface lands and production callsites flip to the engine-backed oracle by default. | Phase 13 Sprint `13.9` (closes with the production-default flip, not the bridge landing) |
