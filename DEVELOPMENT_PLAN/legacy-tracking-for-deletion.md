...
**Current state (2026-08-22): Pending Removal is open with 14 rows; Completed
has 162.** Sprint `269.1` added three rows for the Harbor deprecation and closed
the markdown-link half of the renumber-stale reference row. The 2026-08-19 reference audit added one row: the renumber-stale
phase-document references (24 broken markdown links, 114 stale `Referenced by`
header paths) that `jitml docs check` cannot see because it does not resolve
link targets. Sprint `78.1` closed six on 2026-08-19 — the CUDA renderer's
anonymous namespaces, the non-atomic artifact write on both native lanes, the
Apple arm's fixed `.tmp` staging suffix, the compiler version missing from
artifact identity, the dead `JitToolchainDrift` constructor, and the
triplicated nvcc compile-line prose. The 2026-08-16 documentation-truth pass had added five rows: the
...
remains open and has been **transferred from Sprint `263.1` to Sprint `282.1`**
...
|------|----------|--------------------|---------------|-------------|
| Renumber-stale phase-document references | 9 files with broken markdown links (`documents/engineering/durable_state_dsl.md` ×6, `DEVELOPMENT_PLAN/attestations/{linux-cuda,apple-silicon}-report-card.md` ×4–5, `cluster_topology.md`, `code_quality.md`, `jit_codegen_architecture.md`, `cli_command_surface.md`, `daemon_architecture.md`, `numerical_core.md`), plus 17 files whose `**Referenced by**:` headers name pre-renumber filenames | The 2026-07-24 phase-per-session renumber renamed every `phase-N-*.md` but did not update the citations: **24** markdown links resolve to files that do not exist and **114** header paths name pre-renumber filenames. `jitml docs check` passes regardless because it does not resolve link targets, which is why this survived the renumber. Distinct from citations written `[phase-32-old-name.md](README.md#legacy-to-new-phase-map)`, where the link *text* is deliberately the old filename and the target resolves correctly. | Sprint `278.1` (link half closed by Sprint `269.1`) | **Partly closed 2026-08-22.** Sprint `269.1` extended `jitml docs check` to resolve every `phase-N-slug.md` markdown link destination in a governed document, and repointed the **36** dangling links it found — seven `documents/engineering/` files and both attestations — at `README.md#legacy-to-new-phase-map`, the convention already used inside `DEVELOPMENT_PLAN`. The class can no longer recur for link destinations: the 2026-08-22 `269`–`288` → `270`–`289` renumber added zero. **Still open:** **127** bare `phase-N-*.md` paths in `**Referenced by**:` headers across 20 files, which are metadata rather than link destinations and so are outside the resolver's remit. Same defect class as the `ExternalBars` row below, which that sprint already owns. |
| Harbor container registry and its Percona Postgres | `chart/values/harbor.yaml`, `chart/Chart.yaml` (`harbor` dependency), `chart/templates/httproute-harbor-{api,portal,registry,service}.yaml`, `chart/templates/pv-platform-harbor-pg-{0,repo1-0}.yaml`, vendored `chart/charts/harbor-1.16.2.tgz` and `pg-operator-2.5.1.tgz` | Harbor contributes core, registry, jobservice, portal, nginx, Redis and Trivy pods plus a full Percona Postgres cluster (`harbor-pg`, pgBouncer, pgBackRest repo host) to serve a surface the daemon uses only for push, pull, tag, existence and catalogue listing. Its registry storage is already the MinIO S3 bucket `harbor-registry`, so a `registry:2` deployment reads the same blobs with no migration. `harbor-pg` is the sole row in `postgresRegistry`, so the Percona operator has no other consumer. | Sprint `269.1` | One `registry:2` Deployment and Service on the existing MinIO bucket, no authentication on the local stack; `postgresRegistry` empties and the operator, both PVs, Redis and Trivy are deleted with it. |
| Harbor-proprietary registry API surface | `src/JitML/Service/HarborSubprocess.hs`, `src/JitML/Service/Capabilities.hs` (`HasHarbor`), `src/JitML/Service/Clients.hs`, `src/JitML/Service/BootConfig.hs` (`harborRegistry` Dhall field) | The capability class and its subprocess layer are named for a specific registry product and three of their calls target Harbor's `/api/v2.0` projects/repositories/artifacts endpoints, which no other registry serves. `docker login` exists only because Harbor required credentials. | Sprint `269.1` | `HasImageRegistry` over `RegistrySubprocess`, implemented against the Docker Registry v2 API — `GET /v2/_catalog`, `HEAD /v2/{name}/manifests/{ref}` with the `Docker-Content-Digest` header, and a manifest re-`PUT` for tagging. Push/pull/tag/manifest-inspect stay plain `docker` calls because they are registry-agnostic. |
| Harbor admin portal surface | `src/JitML/Web/AdminPortals.hs`, `web/src/Generated/AdminPortals.purs`, `web/test/Main.purs`, `playwright/jitml-demo.spec.ts` (`jitml-portals-admin-harbor-portal`), `src/JitML/Routes.hs` (`harbor-portal`, `harbor-api`, `harbor-service`) | `registry:2` ships no web UI, so the generated admin portal entry and the browser case asserting it describe a component that will not exist. | Sprint `269.1` | Drop the portal entry and its routes; `/v2` points at the registry Service. The Playwright total is unchanged at **77**: the portal was one entry in the `expected` array inside a single admin-portals test, so removing it removed an assertion rather than a test case. The live component count does move, and is re-issued from a completed lane run rather than hand-edited. |
| Broken `ExternalBars` doc references | `src/JitML/Product/ExternalBars.hs` (module docstring links) | The relative links resolve one directory too high, and they cite `DEVELOPMENT_PLAN/phase-32-external-truth-realness-harness.md`, which the 2026-07-24 renumber renamed to `phase-278-external-bars-no-self-referential-gate-lint-and-exact-served.md`. | Sprint `278.1` | Correct relative depth and the current phase filename. |
...
| Prose/table ProductRow lane-fragment aggregation | Phase `29` / `30` attestation tables and `src/JitML/Test/Report.hs` attestation parsing/join helpers | The freely constructible DTO and prose parsing/join helpers are removed in the current Phase `276` worktree; replacement validation remains open. | Sprint `31.3` / Phase `276` | Complete the typed pinned-journal join and adversarial/CPU validation before moving this row to Completed; aggregation performs no accelerator rerun. |
...
| Pure-gate-only negative-control stand-in and pass-on-pending sentinel | `src/JitML/Test/NegativeControls.hs` (`pendingProductionControls` and the explicit production-hook gap), `test/negative-controls/Main.hs` | Phase `277` is Done for retaining the hand-built pure gate-soundness records, while the test deliberately keeps the production-path controls pending; it does not prove the real workflow rejects a mutated artifact. | Phases `280`–`282` (legacy Sprints `32.4`–`32.6`) | Phase `280` adds request/event mutations, Phase `281` adds journal/reducer fixtures and properties, and Phase `282` adds lifecycle failures plus mandatory per-`ProductRow` registration through the validated workflow contract; a pending control is `NotRun`/failure, never a green production-coverage assertion. |
...
|------|------------|-------|
| Anonymous namespaces in the CUDA layer-training renderer | `src/JitML/Codegen/CudaLayerTraining.hs` | Phase `78` / Sprint `78.1` (2026-08-19) | `cudafe` mangles an anonymous namespace as `_GLOBAL__N__<random>`, so the same rendered source compiled twice produced different artifact bytes. `Codegen/Cuda.hs` and `Codegen/MlpCuda.hs` already use `static` and were already reproducible; this renderer was the sole outlier. `OneDnn.hs` keeps its anonymous namespaces — `g++` mangles them deterministically and Sprint `263.1` pins those artifact bytes. File-scope helpers rendered `static`, with the style read off `JitML.Substrate.profileLinkage` rather than chosen per renderer. |
| Non-atomic artifact write on both native lanes | `src/JitML/Engines/Loader.hs`, `src/JitML/Engines/Engine.hs` | Phase `78` / Sprint `78.1` (2026-08-19) | The compiler wrote directly to the final content-addressed cache path, so a killed or failed compile left a truncated `.so` that the next run's `doesFileExist` reported as a cache hit. Two governed sentences claimed this write was atomic when only the Apple arm was. One `withStagedArtifact` staging+rename shared by both `ArtifactFill` arms. |
| Fixed `.tmp` staging suffix | `src/JitML/Engines/Loader.hs` (`writeTextAtomic`) | Phase `78` / Sprint `78.1` (2026-08-19) | The Apple metadata write staged through a single fixed `<artifact>.tmp` path, so two concurrent writers of one artifact raced on the same staging file and could rename a half-written document into place. A per-invocation staging token, shared with the compile path. |
| Compiler version absent from artifact identity | `src/JitML/Engines/Fingerprint.hs` (`factsCompiler` = the bare literal `"nvcc"`) | Phase `78` / Sprint `78.1` (2026-08-19) | A CUDA toolkit upgrade produced different machine code at an unchanged content address, and the stale artifact was served indefinitely. The version cannot enter the cache key without making the pure fingerprint `IO`. A toolchain sidecar recorded beside each artifact and checked at cache-hit time — a cache-*validity* rule that leaves the published six-tuple untouched. |
| Dead `JitToolchainDrift` error constructor | `src/JitML/AppError/AppError.hs` | Phase `78` / Sprint `78.1` (2026-08-19) | The constructor existed, rendered, and was snapshot-tested, but was constructed at zero sites in `src/` — a declared failure mode for exactly the defect above that nothing could ever raise. Deleted. The delivered cache-validity design makes a toolchain mismatch a recoverable **miss** — the artifact is recompiled at the same content address — so there is no unrecoverable drift state left for an `AppError` to name. Raising one would have turned a self-healing cache into a hard failure. The constructor, its `renderError` arm, its snapshot entry, and its row in the `haskell_code_guide.md` exit-code table are gone, and the two phase documents that enumerate the ADT no longer list it. |
| Triplicated nvcc compile-line prose | `README.md` (toolchain table, codegen bullet), `documents/engineering/determinism_contract.md` | Phase `78` / Sprint `78.1` (2026-08-19) | The pinned CUDA argument list was restated in three governed documents, against `documentation_standards.md` §6 ("never copy doctrine text between docs — cite the owner"). Each copy drifted independently as arguments were added. One owner in `jit_codegen_architecture.md`; the others link to it. |
