...
| Dead deterministic SL pipeline (`SL.Train` + `SL.Loop` curve replay) | `src/JitML/SL/Train.hs`, `src/JitML/SL/Loop.hs` | A second synthetic pipeline that replays `convergenceCurve`, not reachable from `jitml train`; carries the appearance of a real loop. Deleted when the device-convergence assertion covers the cohort through live datasets (Sprint 13.17). | 8.10 |
| `rl rollout` `deterministicTrajectory` LCG (CLI half resolved 2026-06-10) | `src/JitML/RL/Algorithms.hs` `deterministicTrajectory` | The `rl eval` echo and the `rl rollout` LCG are removed from `src/JitML/App.hs`: `jitml rl eval` now shares `runCheckpointEval` (real device forward, missing → `InferenceCheckpointMissing`) and `jitml rl rollout` runs a real on-device PPO rollout (`runDeviceRollout`, fail-closed `InvalidConfig`). The `deterministicTrajectory` LCG symbol itself is still referenced by the canonical determinism test and moves to `Completed` when that test reads a real rollout. Host-clean + container `check-code: ok`. | 9.9 |
| `inferFromManifest` faithful read across the remaining manifest-only sites | `src/JitML/Checkpoint/Store.hs`, `src/JitML/Service/Workload.hs`, `src/JitML/Web/Server.hs` | Sprint 10.5 removed the `+ nTensors/100` synthetic offset: the three engine checkpoint runners now return the faithful kernel output, `inferFromManifest` is an identity read (no fabricated value), and `jitml inference run` / `jitml inspect replay` fail closed / report real manifest metadata instead of a synthetic summary. Routing the remaining manifest-only read sites (Store / Workload / Web demo, the last via Sprint 11.8) through the substrate weighted kernel is the residual work. | 10.5 / 13 |
| Demo endpoint live-cluster round-trip + `/api/images` placeholder | `src/JitML/Web/Server.hs` | Sprint 11.8 routed `/api/inference` through the real policy/value network forward and `/api/connect4/move` through the real MCTS tree search (no hard-coded column, no synthetic `inferFromManifest`). The residual work is the live-cluster round-trip serving a real trained checkpoint over the daemon inference topics (Phase 13) and the `/api/images` upload contract. | 11.8 / 13 |
...
device replacement executing the real oneDNN kernel (`check-code: ok`,
`jitml-sl-canonicals` 19 / 19 with the device-convergence case `OK (1.61s)`,
`jitml-rl-canonicals` 28 / 28 with the on-device PPO case `OK (0.88s)`,
`docs check: ok`):
...
Two rows stay in `Pending Removal`: the synthetic `finalLoss` /
`convergenceCurve` symbols and the `SL.Train` / `SL.Loop` deterministic pipeline
are retained (they no longer feed the published training loss but still back the
deterministic-curve tests); their deletion couples to the device-convergence
assertion landing through the live datasets (Sprint 13.17). The per-trainer
mid-run pure device fallback row also stays pending (Sprint 8.11).
