...
visualizations, and Playwright product proof remain open. Phases `11`–`12` are Active again, Phases `13`–`15`
are Blocked again, and Phases `16`–`18` are added for the full runtime,
...
|---|---|---|---|
| Incomplete visualization and replay renderers | `web/src/Panels/Training.purs`, `web/src/Panels/Rl.purs`, `web/src/Panels/Tune.purs`, `web/src/Panels/Connect4.purs` | Sprint `11.9` now renders loss bars, action-probability bars, replay scrub summaries, MCTS visit/policy/value summaries, multi-game adversarial boards, and tuning heatmap/frontier summaries, but full no-caveat browser closure still needs richer device telemetry, replay artifact selection, RL animation canvases, rules-complete per-game annotations, and transcript-backed adversarial replay. | Sprint `11.9` / Sprint `17.1` |
