...
**Current state (2026-08-12): Pending Removal is open with 13 rows; Completed
has 146.** The 2026-08-12 execution-architecture reopen added six rows for duplicated per-substrate surfaces: three operator vocabularies, three engine drivers, the scattered `Substrate` branches, the duplicate `Substrate` ADT, the hand-written fingerprint prose, and the repeated codegen wrapper text. Those are deletion targets. The defects that reopened the phases — the fail-open catch-alls, the missing per-substrate IR execution, the declaration-derived device evidence, and the unimplemented Dhall layer DSL — are primary unmet obligations and stay in their owning sprints' `### Remaining Work` per rule `C`, not here. Phase `263` removed the provenance-free `CheckpointList` renderer
...
|------|----------|--------------------|---------------|-------------|
| Three parallel operator vocabularies and the dead catalog bridge | `src/JitML/Numerics/Catalog.hs` (`Layer`, 16 constructors), `src/JitML/Numerics/LayerGraph.hs` (`LayerKind`, 12; `LayerOp`, 11), `src/JitML/Codegen/KernelFamily.hs` (`familyForLayer`) | Three hand-maintained vocabularies describe the same operator set. Only the two that cannot execute are checked against each other, and their sole bridge to execution, `familyForLayer`, has no caller anywhere in `src/` or `test/`. A node additionally carries both a `LayerKind` tag and a `LayerOp`, and the device forward dispatches on the tag, so a graph can claim operators it did not run. | Sprint `72.1` | One vocabulary that the executed IR, the catalog, and the Dhall surface all derive from, with the kind tag derived from the operator rather than stored beside it. |
| Three near-identical per-substrate engine drivers | `src/JitML/Engines/Local.hs`, `src/JitML/Engines/CudaLocal.hs`, `src/JitML/Engines/MetalLocal.hs` | About 1,162 lines across three modules, of which roughly 150-190 are genuinely substrate-specific. Two helper pairs are byte-identical between the CPU and CUDA modules, two checkpoint-inference functions diff to zero, and `renderBool` exists in five copies. The variation is data, not behaviour. | Sprint `79.1` | One parameterized driver over a `SubstrateProfile` record whose `launch` field absorbs the `dlopen`/`dlsym` versus fixed-bridge difference. |
| Scattered `Substrate` branches superseded by one profile record | ~82 branching sites across `src/JitML/{Service,Cluster,Plan,Product,Test}/`, `src/JitML/Bootstrap.hs`, `src/JitML/App.hs` | The same per-substrate fact is restated in many places; `substrateHasClusterCompute` alone exists in six independent forms across four shapes. Without `-Werror=incomplete-patterns` a new substrate silently takes a wildcard branch at roughly 57 of those sites. | Sprint `79.1` | One `SubstrateProfile` record and a single total `profileFor`, so adding a substrate fills one record and every consumer is total by construction. |
| Duplicate `Substrate` ADT and its parallel renderer | `src/JitML/Cache/Key.hs` (`Substrate`, `substrateText`, its JSON codec), bridged by `src/JitML/Engines/TuningCache.hs` | A second structurally identical `Substrate` type with its own renderer and codec, alongside a third renderer in the test binaries. Two types and three renderers for one closed set. | Sprint `78.1` | Retain the serialisation type where the on-disk cache key requires it, and derive its rendering from the one shared function instead of restating it. |
| Hand-written toolchain-fingerprint prose | `src/JitML/Engines/{Local,CudaLocal,MetalLocal}.hs`, `src/JitML/Numerics/{MlpOneDnn,MlpCuda,MlpMetal}.hs`, `src/JitML/Numerics/LayerGraphOneDnn.hs`, `src/JitML/App.hs` | Eight fingerprints enumerate C signatures in prose that must be re-synced by hand when an ABI changes. Five signature lines are triplicated character-for-character; one describes a C ABI its artifact does not export; the Metal bridge-ABI token is interpolated in one site and hardcoded in another; and the non-`linux-cpu` build fingerprint names no compiler at all. Only one of the eight is asserted by a test. | Sprint `78.1` | Fingerprints derived from the emitter/primitive set they cover, so a renderer or flag change invalidates its artifact automatically. |
| Repeated codegen wrapper text | `src/JitML/Codegen/{Cuda,OneDnn,Metal}.hs`, plus duplicated `cString`/`escape`/`jsonString`/`sha256HexText` helpers | Roughly 45% of the family renderers is scaffolding or mechanically repeated wrapper text — one CUDA wrapper appears eight times, one Metal signature block nine times — and the emit skeletons are identical but permuted. Two of the three renderers already emit the signature once and splice the body as data. | Sprint `84.1` | One renderer parameterized by a per-substrate emitter record, with each substrate contributing kernel bodies and its own decomposition as data. |
