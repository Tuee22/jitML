...
**2026-07-18 — name-derived supervised writer replacement landed.** Supervised
ProductRow publication now requires the refined training-returned V2 artifact
and non-optional completion, writes one physical `supervised.weights` JMW1
object, and derives its virtual slices from graph order. Generic V1 writing is
retained only as a guarded non-supervised compatibility surface. At that audit
...
production `Local`, `CudaLocal`, and `MetalLocal` V2 executors no longer install
`RuntimeOperations.host*` for transforms or graph operations. Linux CPU and
Linux CUDA use generated content-addressed native ABI kernels; Apple Silicon
uses generated MSL through the fixed bridge with explicit fp32 transport. The
replacement validates ABI/capabilities/symbols and fails through typed errors.
At that audit point the cleanup row remained Pending until the canonical image,
production parity/republishing, and full validation gates passed.
...
| Decorative `archLayerGraph` beside a parallel executable layer/state path | `src/JitML/SL/Architecture.hs`, `src/JitML/Numerics/LayerGraph.hs` | The advertised graph is not the single trained computation: execution and gradients still follow paired `[LayerSpec]` / `[LayerState]` values, with incomplete residual/direct-gradient behavior and silent shape/fallback seams. The earlier completed row therefore did not establish one literal executable graph. | Sprint `23.1` | One typed executable layer IR owns forward, reverse-mode input/parameter gradients, serialization order, and inference reconstruction; unsupported shapes and layer kinds fail explicitly. |
| Per-row artifact-reader and declared-test-id integration cells | `test/integration/Main.hs` (`productRowIntegrationTest`, `productRowIntegrationEvidence`, coverage fixtures), `src/JitML/Test/Report.hs` row integration evidence | The cell reads a pre-published artifact or checks declared IDs instead of executing the row's plan and consuming the resulting completion journal, so a green exit does not prove that cell performed training. | Sprint `28.4` | Contract-driven per-row live execution returning opaque measured `CompletedRunEvidence` for the selected plan. |
...
| Pure-gate-only negative-control stand-in and pass-on-pending sentinel | `src/JitML/Test/NegativeControls.hs` (`pendingProductionControls` and the explicit production-hook gap), `test/negative-controls/Main.hs` | The stanza proves hand-built pure records are rejected, while the test passes when production-path controls remain pending; it does not prove the real workflow rejects a mutated artifact. | Sprint `32.4` | Per-`ProductRow` production-path mutation scenarios executed through the same validated workflow contract; a pending control is `NotRun`/failure, never a green assertion. |
...
| Name-derived generic supervised checkpoint writer | Sprint `10.6` (2026-07-19) | Authoritative supervised publication now requires the refined training-returned V2 artifact and non-optional completion, one physical `supervised.weights` JMW1 object, graph-derived virtual slices, and exact plan/seed/update/dataset/weight/evidence binding. Generic V1 writers remain guarded for non-supervised compatibility only. The final immutable-image run published **11 / 11** exact Product-origin V2 rows, passed the focused pointer proof **1 / 1**, and passed the same full Sprint `10.6` validation gate. |
