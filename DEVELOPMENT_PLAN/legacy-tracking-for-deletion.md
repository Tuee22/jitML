...
| Harbor admin portal surface | `src/JitML/Web/AdminPortals.hs`, `web/src/Generated/AdminPortals.purs`, `web/test/Main.purs`, `playwright/jitml-demo.spec.ts` (`jitml-portals-admin-harbor-portal`), `src/JitML/Routes.hs` (`harbor-portal`, `harbor-api`, `harbor-service`) | `registry:2` ships no web UI, so the generated admin portal entry and the browser case asserting it describe a component that will not exist. | Sprint `269.1` | Drop the portal entry and its routes; `/v2` points at the registry Service. The browser evidence total moves and is re-issued from a completed lane run, never hand-edited. |
