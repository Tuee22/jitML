...
|------|----------|--------------------|---------------|-------------|
| Vestigial `onPolicyTuning` substrate fan-out | `src/JitML/RL/TrainerExecution.hs` (`onPolicyTuning LinuxCPU` / `LinuxCUDA` / `AppleSilicon`) | All three arms return the identical `(10, 5.0e-4)` pair. The comment records that a more aggressive CUDA-only pair previously left PPO/MaskablePPO cartpole stuck at ~210, so the fan-out is retained shape with no retained behaviour — and it invites exactly the per-substrate retune it warns against. | Sprint `265.1` | One substrate-independent constant, so an RL hyperparameter cannot be diverged per lane without deleting the constant deliberately. |
| Unused cuBLAS/cuDNN macros and link flags on the RL MLP artifact | `src/JitML/Engines/Engine.hs` (`-DJITML_USE_CUBLAS=1`, `-DJITML_USE_CUDNN=1`, `-lcublas`, `-lcudnn`) | `src/JitML/Codegen/MlpCuda.hs` references neither macro nor library — it is a hand-written elementwise kernel — so the RL `.so` links two libraries it never calls and the toolchain fingerprint over-constrains that artifact's cache key. | Sprint `265.1` | Per-artifact compile/link flags, so the family kernels keep cuBLAS/cuDNN and the MLP artifact does not. |
| `profileDeterminism` entries that no compile line passes | `src/JitML/Substrate.hs` (`--use_fast_math=false`, `cudnn-explicit-algorithm-id`, `warp-shuffle-deterministic`) | `Engine.compileSubprocess` deliberately omits `--use_fast_math` (modern nvcc rejects the `=false` spelling) and the executed trainer MLP kernel uses neither cuDNN nor warp shuffles. Because this list feeds `buildToolchainFingerprint`, the cache key attests determinism properties of a different kernel. | Sprint `78.1` | Determinism facts derived from the surface they describe, as every other fingerprint input already is. |
| Dead `policySubstrate` field | `src/JitML/RL/Policy.hs` | Set by `defaultPolicy` and never read anywhere in `src/` or `test/`; it implies a per-substrate policy distinction that does not exist. | Sprint `265.1` | Delete the field. |
