...
| `HasMinIO` capability class (`putBlobIfAbsent`, `casPointer`, `listObjects`, `deleteObject`, `minioPutIfAbsent`, `minioReadObject`) plus `ETag` newtype; filesystem-backed instance `JitML.Service.FilesystemMinIO` exercised by `jitml-integration` | Capability Classes and Service Errors | 🔄 Active; missing: live HTTP-backed instance against a running MinIO StatefulSet | Sprint 5.4 |
