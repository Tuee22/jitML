...
|---------|---------|---------|--------|---------------|
| Bootstrap | `jitml bootstrap --apple-silicon\|--linux-cpu\|--linux-cuda` | Plan/Apply substrate bootstrap surface; non-dry-run apply materializes Kind/chart/Dhall/publication files locally and then runs `JitML.Bootstrap.liveExecutePhasedRollout` through typed `kind` / Helm / Docker build and explicit Kind image-load / repo-owned manifest apply / readiness / Pulsar-topic subprocesses with no process-environment gate, writing the leased-port publication with Helm-status-derived component health | ✅ Done; live Linux CPU validation on 2026-05-19 executed the 100-step rollout from `jitml:local`, served edge routes through Envoy, and wrote ready publication health for Harbor, MinIO, Pulsar, Postgres, observability, `jitml-service`, and `jitml-demo` | Sprints 2.1–2.7, 3.5 |
| Service daemon | `jitml service` | Renders daemon lifecycle, BootConfig/LiveConfig, endpoint, logging, retry, at-least-once helper surfaces, BootConfig-derived client acquisition settings, BootConfig-derived daemon Pulsar subscription plans, per-topic startup subscription-acquisition status, combined `DaemonServiceClient` capability interpreter, read-only non-Pulsar client probe status, typed mutating non-Pulsar workload-effect runner plus byte-faithful parsed dispatcher, bounded acquired-subscription consumer batch, and LiveConfig-derived dedup cache size/TTL; starts the in-binary HTTP listener, and the local Helm chart carries the current typed Dhall ConfigMap surface | 🔄 Active; live single-pod `/healthz`, `/readyz`, and `/metrics` are validated, local startup subscription acquisition crosses the typed `HasPulsar.pulsarSubscribe` boundary through `DaemonServiceClient`, the real Linux CPU service pod live-validates acquired MinIO list / Harbor list / in-cluster kubectl probes, `jitml-daemon-lifecycle` validates local MinIO write/CAS, Harbor promote, and kubectl apply/status/delete workload effects plus parsed dispatcher routing through the capability classes, and the bounded daemon batch is validated against a synthetic broker; missing: Pulsar long-lived subscription/redelivery from the running daemon, live invocation of `daemonWorkloadDispatcher` from the service pod, and real training/inference handler integration with those workload effects | Sprints 5.1, 5.4, 5.5, 5.6 |
...
| `HasMinIO` capability class (`minioPutIfAbsent`, `minioReadObject`, `minioReadBytes`, `putBlobIfAbsent`, `putBlobBytesIfAbsent`, `casPointer`, `listObjects`, `deleteObject`) plus `ETag` newtype; filesystem-backed instance `JitML.Service.FilesystemMinIO`, subprocess-backed HTTP S3 instance `JitML.Service.MinIOSubprocess`, daemon acquisition settings, the combined `DaemonServiceClient` interpreter in `JitML.Service.Clients`, and `JitML.Service.Workload` checkpoint blob/pointer effects plus parsed dispatcher routing | Capability Classes and Service Errors | ✅ Done for the standalone capability path and BootConfig-derived settings; 2026-05-19 live Linux CPU validation confirms `If-None-Match: *` and `If-Match` conflicts map to `SEConflict` against the routed MinIO edge; 2026-05-20 live service-pod validation confirms the acquired daemon client lists `jitml-checkpoints`; local workload payloads invoke MinIO write/CAS through the capability class; live service-pod routing of real workload writes remains Phase 5 work | Sprint 5.4 |
| `HasPulsar` capability class (`pulsarPublish`, `pulsarAcknowledge`, `pulsarSubscribe`, `pulsarConsume`, `pulsarSeek`) plus `SubscriptionId` newtype and routed one-shot WebSocket instance `JitML.Service.PulsarWebSocketSubprocess`; `JitML.Service.Clients` derives cluster and Apple-host WebSocket endpoints from `BootConfig` and exposes them through `DaemonServiceClient`; `JitML.Service.Consumer` derives the daemon topic subscription plan from `BootConfig`; `DaemonRuntime` renders the planned subscriptions under `pulsar_subscriptions` and acquisition results under `pulsar_subscription_status` | Capability Classes and Service Errors | 🔄 Active; standalone publish/consume through live Pulsar is validated, daemon settings are derived, local `pulsarSubscribe` planning is tested, `jitml service` crosses the startup subscription-acquisition boundary through a routed WebSocket consumer-open probe via `DaemonServiceClient`, and local dispatch failure invokes `pulsarSeek` without poisoning the dedup cache; missing: daemon-acquired long-lived subscription plus live post-dispatch ack/redelivery/seek semantics | Sprints 4.4, 5.4, 5.5 |
| `HasHarbor` capability class (`harborImageExists`, `harborPromoteImage`, `harborPushImage`, `harborPullImage`, `harborListImages`) | Capability Classes and Service Errors; subprocess-backed explicit local settings live in `JitML.Service.HarborSubprocess`; `JitML.Service.Clients` derives registry/API settings from `BootConfig.harborRegistry`; `JitML.Service.Workload` invokes image promotion through parsed dispatcher routing | ✅ Done for the standalone capability path and BootConfig-derived settings; typed Docker/curl command rendering and live push/pull/list/artifact-existence against Harbor are validated; 2026-05-20 live service-pod validation confirms the acquired daemon client lists the Harbor `library` project; local workload payloads invoke Harbor promotion; live service-pod routing of real push/pull/promote operations remains Phase `5` work | Sprint 5.4 |
| `HasKubectl` capability class (`kubectlApply` with stdin-piped YAML via `subprocessWithStdin`, `kubectlStatus`, `kubectlGet`, `kubectlDelete`); subprocess-backed instance `JitML.Service.KubectlSubprocess` pins `./.build/jitml.kubeconfig` for host clients and omits `--kubeconfig` for in-cluster service-account clients; `JitML.Service.Clients` carries those settings into `DaemonRuntime`; `JitML.Service.Workload` invokes apply/status/delete through parsed dispatcher routing | Capability Classes and Service Errors | ✅ Done for typed command shape and daemon settings; 2026-05-20 live service-pod validation confirms in-cluster `kubectl get pods` through the `jitml-service` service account; local workload payloads invoke apply/status/delete; live service-pod routing of real Kubernetes workload operations remains Sprint `5.4` work | Sprint 5.4 |
| `RetryPolicy` typed value | Retry Policy as First-Class Values | ✅ Done | Sprint 5.4 |
| At-least-once Pulsar consumer | At-Least-Once Event Processing | 🔄 Active; typed dispatcher (`JitML.Service.Consumer.{consumerStep,runConsumerLoop,consumerOutcomeError}`), fully-qualified topic routing, BootConfig-derived subscription planning, `subscribeDaemonTopics`, startup acquisition (`JitML.Service.Runtime.acquireDaemonSubscriptions`), bounded acquired-subscription batch (`JitML.Service.Runtime.daemonConsumerBatch`), WebSocket subscribe-probe rendering, LiveConfig-derived dedup cache size/TTL, TTL-boundary expiry, dispatch-failure seek-without-dedup-poisoning, and lifecycle-exit wiring (`JitML.Service.Runtime.consumerLoopExit`) are validated locally; missing: daemon long-lived Pulsar subscription, live post-dispatch ack/redelivery/seek, and LRU dedup cache populated by live events | Sprint 5.5 |
...
| Stateless `Deployment` (not `StatefulSet`) | Long-Running Daemons | 🔄 Active; live single-replica readiness is validated through the live bootstrap path and live two-replica required anti-affinity is validated on a two-worker Kind cluster; missing: real long-lived Pulsar subscription from the running pod | Sprint 5.6 |
...
| Capability classes (`HasMinIO`, `HasPulsar`, `HasHarbor`, `HasKubectl`) — full doctrine-required methods checked in | Capability Classes and Service Errors | 🔄 Active; `HasMinIO`, one-shot `HasPulsar`, `HasHarbor`, and `HasKubectl` subprocess instances are live-validated, `DaemonServiceClient` exposes all four class instances from BootConfig-derived settings, startup acquisition calls the typed `HasPulsar.pulsarSubscribe` boundary in local tests with WebSocket subscribe-probe rendering, the real Linux CPU daemon pod live-validates acquired MinIO list, Harbor list, and in-cluster `kubectl get pods` probes, and `JitML.Service.Workload` locally parses and dispatches MinIO write/CAS, Harbor promote, and kubectl apply/status/delete payloads; missing: live running-daemon routing of real workload payloads through those non-Pulsar effects and long-lived Pulsar ack/seek semantics | Sprints 4.4, 5.4, 5.5 |
| `RetryPolicy` typed value with named strategies | Retry Policy as First-Class Values | ✅ Done | Sprint 5.4 |
| At-least-once Pulsar consumer with protobuf-message-hash deduplication | At-Least-Once Event Processing | 🔄 Active; local dispatcher, fully-qualified topic routing, BootConfig-derived subscription planning, startup acquisition status, bounded acquired-subscription daemon batch, WebSocket subscribe-probe rendering, LiveConfig-derived dedup cache size/TTL, TTL-boundary expiry, dispatch-failure seek-without-dedup-poisoning, and synthetic subscribe/consume/ack/dedup tests are checked in; missing: daemon long-lived Pulsar subscription, live post-dispatch ack/redelivery/seek, and LRU dedup cache populated by live messages | Sprint 5.5 |
| Long-running daemon shape (`BootConfig` / `LiveConfig`, SIGHUP, `/healthz`, `/readyz`, `/metrics`, structured JSON stderr logging) | Long-Running Daemons in the Same Binary | 🔄 Active; live Linux CPU validation confirms `/healthz`, `/readyz`, `/metrics`, and read-only acquired-client probes from the running daemon; missing: long-lived Pulsar event flow | Sprints 5.2, 5.3 |
