...
| Service daemon | `jitml service` | Renders daemon lifecycle, BootConfig/LiveConfig, endpoint, logging, retry, at-least-once helper surfaces, BootConfig-derived client acquisition settings, BootConfig-derived daemon Pulsar subscription plans, per-topic startup subscription-acquisition status, combined `DaemonServiceClient` capability interpreter, read-only non-Pulsar client probe status, bounded acquired-subscription consumer batch, and LiveConfig-derived dedup cache size/TTL; starts the in-binary HTTP listener, and the local Helm chart carries the current typed Dhall ConfigMap surface | 🔄 Active; live single-pod `/healthz`, `/readyz`, and `/metrics` are validated, local startup subscription acquisition crosses the typed `HasPulsar.pulsarSubscribe` boundary through `DaemonServiceClient`, the real Linux CPU service pod live-validates acquired MinIO list / Harbor list / in-cluster kubectl probes, and the bounded daemon batch is validated against a synthetic broker; missing: Pulsar long-lived subscription/redelivery from the running daemon, live MinIO write/CAS from workload handlers, live Harbor image operations from workload handlers, live kubectl workload operations from the running daemon | Sprints 5.1, 5.4, 5.5, 5.6 |
...
| Tune | `jitml tune` | Current command decodes `experiments/mnist-tune.dhall` through `JitML.Tune.Catalog`, renders `sampler: TPE`, and emits deterministic local trial samples from the full target sampler catalog | 🔄 Active; missing: proto-lens tune bindings, live `tune.command.<mode>` / `tune.event.<mode>` Pulsar round-trip, live trial persistence in MinIO | Sprint 9.7 |
...
| Test runner | `jitml test all` / `jitml test <stanza>` | `--dry-run` renders the aggregate plan; non-dry-run invokes `cabal test all` or `cabal test <stanza>` through typed `Subprocess`, parses the `cabal.project` report-card knob block, then emits the current placeholder-backed report-card summary on success | 🔄 Active; 2026-05-19 `jitml:local` validation passed non-dry-run `jitml test all` across all ten stanzas; missing: live `jitml-e2e` scheduling and report-card values sourced from real SL/RL/AlphaZero/tuning/cross-substrate measurements | Sprint 12.9 |
...
| Bundle output | `src/JitML/Web/Bundle.hs`; typed bundle asset manifest and demo route manifest for the generated PureScript bundle output paths; `src/JitML/Web/Server.hs` serves `web/dist/Main/index.js` as `/bundle/main.js` when the file exists and exposes deterministic local stream routes matching the generated contracts | 🔄 Active; missing: demo image `spago` bundle build and live `/api/ws` proxy | Sprint 11.4 |
...
| Checkpoint store | `jitml service` (training path); local interpreter in `JitML.Checkpoint.Store` | MinIO `jitml-checkpoints/<experiment-hash>/{blobs,manifests,pointers}`; local tests use a filesystem object root with the same key layout | Concurrency: write-once + If-Match CAS on pointers; local tests exercise write-once objects, manifest writes, manifest discovery, latest pointer CAS, GC planning, and inference from latest |
