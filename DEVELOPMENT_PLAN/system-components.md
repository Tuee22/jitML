...
| Demo executable shim | `jitml-demo` | Sibling binary that prints `demoStatusLine` and serves the frontend/API route surface through `src/JitML/Web/Server.hs` | 🔄 Active; missing: compiled PureScript bundle served from the shim, live `/api/ws` proxy, real panel state from the daemon | Sprint 11.5 |
...
| `HasMinIO` capability class (`putBlobIfAbsent`, `casPointer`, `listObjects`, `deleteObject`, `minioPutIfAbsent`, `minioReadObject`) plus `ETag` newtype | Capability Classes and Service Errors | 🔄 Active; missing: live instance against a running MinIO StatefulSet, `If-Match` 412 → `SEConflict` integration coverage | Sprint 5.4 |
...
| `HasKubectl` capability class (`kubectlApply`, `kubectlStatus`, `kubectlGet`, `kubectlDelete`) | Capability Classes and Service Errors | 🔄 Active; missing: live `kubectl` invocation through the typed `Subprocess` boundary | Sprint 5.4 |
