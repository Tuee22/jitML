...
| Phased deploy | Target order is Harbor first → mirror/build into Harbor → final services; current code renders the ordered plan, materializes local inputs, and emits typed mirror subprocesses via `src/JitML/Cluster/DockerImage.hs` (`dockerBuildSubprocess`, `dockerTagSubprocess`, `dockerPushSubprocess`, `dockerLoginSubprocess`, `dockerMirrorPlan`) | 🔄 Active; mirror plan emission validated by `jitml-integration`; missing: live execution of the phased apply (Harbor + Helm) against a real Kind cluster, mirror plan wired into the live executor | Sprint 3.5 |
