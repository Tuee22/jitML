...
runtime behaviour that the worktree does not yet exercise. Components move from
...
| Linux CPU | `linux-cpu` | generated oneDNN-style C++ source under `./.build/jit-src/linux-cpu/<hash>/`, plus local identity and reduction-smoke compile/load/run through `JitML.Engines.Local` | fully containerized target: `jitml:local` | target one daemon: `Cluster + SelfInference`; current code renders configs and summaries and validates same-host identity plus reduction-smoke kernel paths | 🔄 Active; missing: real oneDNN graph wrappers beyond the local smoke kernels | [Phase 7](phase-7-jit-codegen-and-substrates.md) |
| Linux CUDA | `linux-cuda` | generated CUDA C source under `./.build/jit-src/linux-cuda/<hash>/` | fully containerized target: `jitml:local` (CUDA activates at runtime when scheduled to `runtimeClassName: nvidia`) | target one daemon: `Cluster + SelfInference`, pod anti-affinity = one per node; current code renders configs and summaries; live `RuntimeClass/nvidia` GPU visibility is validated | 🔄 Active; missing: CUDA compile/load and cuBLAS/cuDNN execution | [Phase 7](phase-7-jit-codegen-and-substrates.md) |
...
|---------|---------|---------|--------|---------------|
| Bootstrap | `jitml bootstrap --apple-silicon\|--linux-cpu\|--linux-cuda` | Plan/Apply substrate bootstrap surface; non-dry-run apply materializes Kind/chart/Dhall/publication files locally and then runs `JitML.Bootstrap.liveExecutePhasedRollout` through typed `kind` / Helm / Docker build and explicit Kind image-load / repo-owned manifest apply / readiness / Pulsar-topic subprocesses with no process-environment gate, writing the leased-port publication with Helm-status-derived component health | ✅ Done; live Linux CPU validation on 2026-05-19 executed the 100-step rollout from `jitml:local`, and the 2026-05-20 live run executed the current 110-step rollout, served edge routes through Envoy, and wrote ready publication health for Harbor, MinIO, Pulsar, Postgres, observability, `jitml-service`, and `jitml-demo` | Sprints 2.1–2.7, 3.5 |
| Service daemon | `jitml service` | Renders daemon lifecycle, BootConfig/LiveConfig, endpoint, logging, retry, at-least-once helper surfaces, BootConfig-derived client acquisition settings, BootConfig-derived daemon Pulsar subscription plans, per-topic startup subscription-acquisition status, combined `DaemonServiceClient` capability interpreter, read-only non-Pulsar client probe status, typed mutating workload-effect runner plus byte-faithful parsed dispatcher and Training/RL/Tune/Inference command-envelope dispatcher, bounded acquired-subscription consumer batch exposed by `--consume-once <n>`, normal-service held-open WebSocket consumer workers with one shared process-lifetime `HandlerRouter`, post-dispatch WebSocket ack and negative-ack surfaces, and LiveConfig-derived dedup cache size/TTL; starts the in-binary HTTP listener, and the local Helm chart carries the current typed Dhall ConfigMap surface | 🔄 Active; live single-pod `/healthz`, `/readyz`, and `/metrics` are validated, local startup subscription acquisition crosses the typed `HasPulsar.pulsarSubscribe` boundary through `DaemonServiceClient`, the real Linux CPU service pod live-validates acquired MinIO list / Harbor list / in-cluster kubectl probes, the service account applies/reads/deletes command-shaped Training/RL/Tune Jobs from inside the pod, 2026-05-21 live `--consume-once` from the service pod dispatches Training/RL/Tune/Inference messages before ack, applies Training/RL/Tune Jobs, routes `WriteCheckpointBlob` effects into MinIO, routes `PromoteWorkloadImage` effects into Harbor same-repository tag promotion, and handles `RunInference` through MinIO checkpoint reads plus Pulsar `InferenceResult` publication, 2026-05-21 live normal-service validation publishes a `RunInference` request and consumes the expected `InferenceResult` through the held-open worker path without using `--consume-once`, 2026-05-21 live duplicate-payload validation produces exactly one matching `InferenceResult`, 2026-05-21 live dispatch-failure validation negative-acks a missing-checkpoint request until seeding the checkpoint lets broker redelivery publish `InferenceResult`, and 2026-05-21 live Linux CUDA validation schedules the actual `jitml-service` Deployment with `runtimeClassName: nvidia` onto `jitml-linux-cuda-worker` and confirms `nvidia-smi -L` reports the RTX 5090 inside that service container; `jitml-daemon-lifecycle` validates local MinIO write/CAS, Harbor promote, kubectl apply/status/delete, RunInference, parsed WorkloadEffect routing, parsed Training/RL/Tune/Inference command envelopes, and the bounded daemon batch against a synthetic broker; `jitml-integration` validates the `--consume-once` CLI surface, zero-queue WebSocket subscribe probe, bare public/default producer topic resolution, post-dispatch WebSocket ack command shape, and held-open worker command shape; the normal service path now starts per-acquired-subscription held-open workers and retains dedup state for the process lifetime; blocked: Apple Silicon host live work | Sprints 5.1, 5.4, 5.5, 5.6 |
...
| Train | `jitml train` | Current command renders a deterministic local canonical-problem summary; `JitML.Proto.Training` renders/parses the current text `StartTraining` / `StopTraining` command envelopes | 🔄 Active; missing: binary proto-lens codecs and real Pulsar `training.command.<mode>` publish → daemon `TrainingHandler` → real loop → `training.event.<mode>` round-trip | Sprint 8.2 |
...
| Inference | `jitml inference run` | Current command runs deterministic `inferFromManifest` summary against a local manifest value; `jitml-integration` validates a filesystem-backed latest-pointer read through `loadInferenceCheckpointWith` and the local Linux CPU generated-kernel FFI runner, while Phase `5.4` live daemon validation covers the default latest-checkpoint read against in-cluster MinIO | 🔄 Active; missing: user-facing live `jitml inference run` MinIO path and real weight-blob loading into production kernel handles | Sprint 10.4 |
...
|-----------|----------------|--------|---------------|
| Per-substrate Kind config | `./kind/cluster-{apple-silicon,linux-cpu,linux-cuda}.yaml` rendered from `JitML.Cluster.Kind`; default one-worker configs plus `kindConfigWithWorkerCount` for live placement validation | ✅ Done | Sprint 3.1 / Sprint 5.6 |
...
| `jitml-service` Deployment | `chart/templates/deployment-jitml-service.yaml`; local chart ConfigMap uses the current typed Dhall union constructors for residency/inference mode and current `LiveConfig` fields; Deployment uses required hostname anti-affinity, `maxSurge: 0` / `maxUnavailable: 1`, and a `jitml-service` service account/RoleBinding | (internal target Pulsar consumer) | 🔄 Active; live single-replica Deployment readiness plus `/healthz`, `/readyz`, `/metrics`, single-worker replacement rollout, service-account kubectl access, and bounded service-pod `--consume-once` dispatch are validated; live two-replica anti-affinity on a two-worker Kind cluster is validated; normal service startup now creates held-open consumer workers, 2026-05-21 live validation proves `RunInference`, duplicate-payload dedup, and dispatch-failure negative-ack redelivery from the running Deployment, and 2026-05-21 live Linux CUDA validation proves the service pod runs on the GPU-labelled worker with `runtimeClassName: nvidia` and sees the RTX 5090 through `nvidia-smi -L`; blocked: Apple Silicon host live work | Sprint 5.6 |
...
| NVIDIA RuntimeClass (CUDA) | `chart/templates/runtimeclass-nvidia.yaml`; `JitML.Cluster.Kind` renders the Linux CUDA worker containerd `nvidia` runtime handler, repo-owned NVIDIA runtime config, read-only `/run/nvidia/driver` host driver-root mount, and toolkit support mounts | (RuntimeClass — selected by the Linux CUDA Deployment podSpec) | ✅ Done; clean live CUDA Kind validation on 2026-05-20 schedules `pod/nvidia-smi-probe` through `RuntimeClass/nvidia` onto the labelled worker and logs `GPU 0: NVIDIA GeForce RTX 5090`; 2026-05-21 live validation schedules the actual `jitml-service` Deployment through the same RuntimeClass on `jitml-linux-cuda-worker` and confirms `nvidia-smi -L` reports the RTX 5090 inside the service container | Sprint 4.7 / Sprint 5.6 |
...
| Stateless `Deployment` (not `StatefulSet`) | Long-Running Daemons | 🔄 Active; live single-replica readiness is validated through the live bootstrap path, live two-replica required anti-affinity is validated on a two-worker Kind cluster, normal service startup creates held-open consumer workers, live Linux CPU redelivery/dedup is validated, and live Linux CUDA service-pod RuntimeClass execution is validated; blocked: Apple Silicon host live work | Sprint 5.6 |
| Required pod anti-affinity (`topologyKey: kubernetes.io/hostname`) | Long-Running Daemons | ✅ Done; rendered with `requiredDuringSchedulingIgnoredDuringExecution`, validated locally, and live-validated on 2026-05-20 by scaling `jitml-service` to two replicas on temporary two-worker Kind cluster `jitml-phase5-affinity` | Sprint 5.6 |
...
| Linux CPU engine (oneDNN codegen + local FFI fixture) | `src/JitML/Engines/{Engine,Local}.hs`, `src/JitML/Codegen/{RuntimeSource,OneDnn}.hs`; generated oneDNN C++ inputs under `./.build/jit-src/linux-cpu/<hash>/`; local identity and reduction-smoke kernels compile with `g++`, load with `dlopen`, and run through the Haskell FFI | 🔄 Active; missing: real oneDNN graph wrappers beyond the smoke kernels, full `HasEngine` production loading | Sprints 7.3, 7.7 |
| Linux CUDA engine (CUDA C codegen) | `src/JitML/Engines/Engine.hs`, `src/JitML/Codegen/{RuntimeSource,Cuda}.hs`; generated CUDA inputs under `./.build/jit-src/linux-cuda/<hash>/` | 🔄 Active; missing: host `nvcc`, cuBLAS/cuDNN bindings, splitmix RNG, FFI loading, and live CUDA transcript determinism | Sprints 7.4, 7.7 |
...
| Hardware auto-tuning | `src/JitML/Engines/Tuning.hs`; `TuningChoice` is a cache-key input and generated-source metadata string; per-substrate knob spaces, deterministic defaults, deterministic-only cuDNN choices, and stable benchmark candidate plans are checked in | 🔄 Active; missing: benchmark-driven on-hardware ranking/persistence and cross-substrate equality beyond the local Linux CPU identity fixture | Sprint 7.6 |
...
| Inference-only read path | `src/JitML/Checkpoint/{Format,Store}.hs` plus `src/JitML/Engines/Local.hs`; deterministic `inferFromManifest`, `loadInferenceCheckpoint`, `loadInferenceCheckpointWith`, and `inferWeightsOnlyFromLatestCheckpoint` cover the latest-pointer → manifest → weight-only inference helper, including a Linux CPU generated-kernel FFI runner in `jitml-integration` and 2026-05-21 live daemon validation against in-cluster MinIO | 🔄 Active; missing: real weight-blob loading into production per-substrate kernel handles | Sprint 10.4 |
...
| Long-running daemon shape (`BootConfig` / `LiveConfig`, SIGHUP, `/healthz`, `/readyz`, `/metrics`, structured JSON stderr logging) | Long-Running Daemons in the Same Binary | 🔄 Active; live Linux CPU validation confirms `/healthz`, `/readyz`, `/metrics`, read-only acquired-client probes, held-open fresh-message consumption, duplicate-payload dedup, and dispatch-failure negative-ack redelivery from the running daemon, and live Linux CUDA validation confirms the service pod runs through the NVIDIA RuntimeClass on the GPU-labelled worker; blocked: Apple Silicon host live work | Sprints 5.2, 5.3, 5.6 |
...
| Generated JIT compiler inputs | `jitml` Haskell runtime source renderers | `./.build/jit-src/<substrate>/<hash>/` | CUDA, oneDNN C++, and Metal / Swift inputs are generated on demand; the local Linux CPU identity and reduction-smoke paths compile and load from this tree |
...
| JIT source generation | `src/JitML/Codegen/{RuntimeSource,Cuda,OneDnn,Metal,SourceFile}.hs`, `src/JitML/Engines/Local.hs`, `./.build/jit-src/<substrate>/<hash>/` | Haskell-generated compiler inputs for CUDA, oneDNN C++, and Metal / Swift; local Linux CPU identity and reduction-smoke compile/load/run |
