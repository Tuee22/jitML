...
checked-in implementation. Phases `0`, `1`, `2`, `3`, `4`, and `6` are
...
Sprint `5.6`'s Linux CPU and Linux CUDA service-pod validations both closed on
2026-05-23 against the single-node topology; Phase `5` remains `🔄 Active`
only for the Apple Silicon host-Dhall service-pod validation. Phases `7`, `8`, `9`,
`10`, `11`, and `12`
...
| Linux CPU | `linux-cpu` | generated oneDNN-style C++ source under `./.build/jit-src/linux-cpu/<hash>/`, plus local identity, reduction-smoke, and family-scaffold compile/load/run through `JitML.Engines.Loader` and `JitML.Engines.Local` with exported `jitml_kernel_family_name` and `jitml_kernel_output_count` verification; local cache keys include `artifact-abi=<os>-<arch>` and `reduction-block=256` in the toolchain fingerprint; `JitML.Engines.CpuFeatures` probes AVX2/AVX-512 through typed subprocesses for the oneDNN micro-kernel knob; `JitML.Engines.OneDnnRuntime` probes `pkg-config` / dynamic-linker `libdnnl` visibility through typed subprocesses; `jitml service` routes `linux-cpu` + `SelfInference` requests through `runLinuxCpuCheckpointInference` after MinIO latest-pointer/manifest loading | fully containerized target: `jitml:local` | target one daemon: `Cluster + SelfInference`; current code renders configs and summaries, validates same-host identity plus reduction-smoke and family-scaffold kernel paths, and wires daemon inference dispatch to the generated Linux CPU FFI runner | 🔄 Active; missing: real oneDNN graph wrappers beyond the local smoke kernels | [Phase 7](phase-7-jit-codegen-and-substrates.md) |
| Linux CUDA | `linux-cuda` | generated CUDA C source under `./.build/jit-src/linux-cuda/<hash>/`; host SplitMix64 RNG surface, generated no-curand metadata, deterministic reduction source with no `atomicAdd`, host reduction partial-count/finalization helpers, typed CUDA runtime probes for `nvcc`, `nvidia-smi`, and dynamic-linker visibility, and generated family/output-count metadata symbols; 2026-05-23 live single-node `RuntimeClass/nvidia` probe on a GPU validation host (RTX 5090) confirms node containerd `nvidia` runtime handler, `/run/nvidia/driver` mount, and pod-visible GPU through `nvidia-smi -L` | fully containerized target: `jitml:local` (CUDA activates at runtime when scheduled to `runtimeClassName: nvidia`) | target one daemon: `Cluster + SelfInference`, pod anti-affinity = one per node; current code renders configs and summaries; live `RuntimeClass/nvidia` GPU visibility validated on single-node topology | 🔄 Active; missing: CUDA compile/load and cuBLAS/cuDNN execution (Phase `7`) | [Phase 7](phase-7-jit-codegen-and-substrates.md) |
...
| Service daemon | `jitml service` | Renders daemon lifecycle, BootConfig/LiveConfig, endpoint, logging, retry, at-least-once helper surfaces, BootConfig-derived client acquisition settings, BootConfig-derived daemon Pulsar subscription plans, per-topic startup subscription-acquisition status, combined `DaemonServiceClient` capability interpreter, read-only non-Pulsar client probe status, typed mutating workload-effect runner plus byte-faithful parsed dispatcher and Training/RL/Tune/Inference command-envelope dispatcher, bounded acquired-subscription consumer batch exposed by `--consume-once <n>`, normal-service held-open WebSocket consumer workers with one shared process-lifetime `HandlerRouter`, post-dispatch WebSocket ack and negative-ack surfaces, and LiveConfig-derived dedup cache size/TTL; starts the in-binary HTTP listener, and the local Helm chart carries the current typed Dhall ConfigMap surface | 🔄 Active; local daemon lifecycle, capability, dispatcher, ack/negative-ack, and generated BootConfig coverage remains validated, but Sprint `5.6` must repeat live single-node Linux CPU rollout, Linux CUDA service-pod `RuntimeClass/nvidia` execution on `jitml-linux-cuda-control-plane` (the underlying Phase `4` Sprint `4.7` RuntimeClass probe closed on 2026-05-23), and Apple Silicon host-Dhall validation | Sprints 5.1, 5.4, 5.5, 5.6 |
...
| Stateless `Deployment` (not `StatefulSet`) | Long-Running Daemons | 🔄 Active; rendered as a stateless single-replica local Deployment with held-open consumer workers, but live single-node rollout, CUDA RuntimeClass execution, and Apple host Dhall validation must be repeated after the topology change | Sprint 5.6 |
| Required pod anti-affinity (`topologyKey: kubernetes.io/hostname`) | Long-Running Daemons | 🔄 Active; rendered with `requiredDuringSchedulingIgnoredDuringExecution` to enforce one service pod per node in non-local multi-node environments. The supported local Kind topology is single-node and therefore supports one local service replica; live Sprint `5.6` validation now targets that local shape | Sprint 5.6 |
...
| Linux CPU engine (oneDNN codegen + local FFI fixture) | `src/JitML/Engines/{CpuFeatures,Engine,HasEngine,Loader,Local,OneDnnRuntime,TuningBenchmark}.hs`, `src/JitML/Codegen/{RuntimeSource,OneDnn}.hs`; generated oneDNN C++ inputs under `./.build/jit-src/linux-cpu/<hash>/`; local identity, reduction-smoke, and family-scaffold kernels compile with `g++` through the shared cache artifact loader, load with `dlopen`, run through the Haskell FFI, report the loaded `jitml_kernel_family_name` plus `jitml_kernel_output_count`, dispatch generated-family requests through the local Linux CPU `HasEngine` interpreter, include `artifact-abi=<os>-<arch>` and `reduction-block=256` in the local toolchain fingerprint, probe AVX2/AVX-512 through typed subprocesses for the oneDNN micro-kernel knob, probe `libdnnl` package/link visibility through typed subprocesses, and measure Linux CPU benchmark candidates through generated FFI output digests | 🔄 Active; missing: real oneDNN graph wrappers beyond the smoke kernels and production service loading | Sprints 7.3, 7.6, 7.7 |
| Linux CUDA engine (CUDA C codegen) | `src/JitML/Engines/{CudaRuntime,Engine,Rng}.hs`, `src/JitML/Codegen/{RuntimeSource,Cuda}.hs`; generated CUDA inputs under `./.build/jit-src/linux-cuda/<hash>/`; host SplitMix64 RNG, generated no-curand metadata, deterministic no-atomic reduction source, host reduction partial-count/finalization helpers, typed `nvcc` / `nvidia-smi` / `ldconfig` CUDA runtime probes, and generated family/output-count metadata are checked in | 🔄 Active; missing: host `nvcc`, cuBLAS/cuDNN bindings, stochastic-kernel RNG ABI consumption, FFI loading, and live CUDA transcript determinism | Sprints 7.4, 7.7 |
...
| Hardware auto-tuning | `src/JitML/Engines/{Tuning,TuningBenchmark,TuningStore,TuningCache}.hs`; `TuningChoice` is a cache-key input and generated-source metadata string; per-substrate knob spaces, deterministic defaults, deterministic-only cuDNN choices, stable benchmark candidate plans, pure measured-result selection/ranking, generic measurement collection with SHA-256 output digests, Linux CPU generated-FFI candidate measurement, guarded CUDA/Metal runtime-preflight runner boundaries, JSON persistence of selected measured choices by substrate/base hash, and persisted-choice cache-key derivation are checked in | 🔄 Active; missing: real oneDNN graph measurements, live CUDA/Metal candidate measurement behind the preflight runners, first-cache-miss hardware invocation, and cross-substrate equality beyond the local Linux CPU family-scaffold fixture | Sprint 7.6 |
...
| Long-running daemon shape (`BootConfig` / `LiveConfig`, SIGHUP, `/healthz`, `/readyz`, `/metrics`, structured JSON stderr logging) | Long-Running Daemons in the Same Binary | 🔄 Active; local lifecycle, endpoint, logging, signal, and held-open worker surfaces are implemented, but live Linux CPU, Linux CUDA, and Apple Silicon validations must be repeated on the single-node substrate topology | Sprints 5.2, 5.3, 5.6 |
...
| `jitml-integration` | `test/integration/Main.hs` covers typed subprocess execution, renderer suites, route-table golden fixture, real-binary spawn matrix, filesystem-backed `HasMinIO` checkpoint snapshot write / inference / resume coverage, local Linux CPU checkpoint inference through a generated FFI kernel, decoded `.jmw1` weights passed into the weighted local inference runner, routed MinIO and Pulsar subprocess command rendering, BootConfig-derived daemon client settings, Dhall numerics decode, and typed live-rollout command shapes | Live checkpoint round-trip, live daemon service capability effects, and per-substrate determinism | 🔄 Active; missing: live checkpoint round-trip, running-daemon Pulsar / cluster effects, and real generated-kernel determinism beyond local Linux CPU smoke kernels | Sprint 12.2 |
| `jitml-sl-canonicals` | `test/sl-canonicals/Main.hs` covers eleven deterministic synthetic convergence curves, their committed per-problem fixtures under `test/golden/sl/<problem-key>/curve.txt`, dataset `HasMinIO` fetch/verification, `TrainingCommand` text render/parse round-trips, and `TrainingCommand` / `TrainingEvent` proto3-compatible byte round-trips | Live SL convergence and per-distribution regression detection against real datasets | 🔄 Active; missing: real training runs, live measured convergence fixtures, and `sl_epochs` / `sl_batch` knob consumption | Sprint 12.3 |
| `jitml-rl-canonicals` | `test/rl-canonicals/Main.hs` covers algorithm metadata, deterministic trajectory helper, PPO/CartPole golden trajectory, all four AlphaZero transcript fixtures, `RlCommand` text render/parse round-trips, and `RlCommand` / `RlEvent` proto3-compatible byte round-trips | RL target matrix forms (2) and (3): same-substrate trajectory determinism plus per-seed final-reward distribution against live committed fixtures | 🔄 Active; missing: same-substrate live trajectory determinism, per-seed final-reward distribution, and measured AlphaZero arena fixtures | Sprint 12.4 |
| `jitml-hyperparameter` | `test/hyperparameter/Main.hs` covers the full target sampler catalog, scheduler / pruner axes including TPE, deterministic trial values, TPE worked-example Dhall decode, sampler-label parsing, Sobol/GA golden fixtures, report-card knob consumption, `TuneCommand` text render/parse round-trips, and `TuneCommand` / `TuneEvent` proto3-compatible byte round-trips | Per-sampler reproducibility, per-scheduler reproducibility, per-pruner reproducibility, and resume-from-partial-sweep equality against live storage | 🔄 Active; missing: per-sampler / per-scheduler / per-pruner reproducibility against live trial storage in MinIO | Sprint 12.5 |
| `jitml-cross-backend` | `test/cross-backend/Main.hs` covers engine determinism flags, checkpoint inference summaries, generated Linux CPU identity/reduction-smoke/family-scaffold kernel compile/load/run, loaded family/output-count symbol checks, and local Linux CPU `HasEngine` dispatch over a generated family kernel | Cross-substrate cohort `(cpu, cuda)` and `(cpu, metal)` on the SL canon; per-tensor drift fits the committed tolerance band per [../documents/engineering/determinism_contract.md](../documents/engineering/determinism_contract.md) | 🔄 Active; missing: live cross-substrate runs against committed `test/golden/cross-backend/` tolerance fixtures | Sprint 12.6 |
...
| Generated JIT compiler inputs | `jitml` Haskell runtime source renderers | `./.build/jit-src/<substrate>/<hash>/` | CUDA, oneDNN C++, and Metal / Swift inputs are generated on demand; the local Linux CPU identity, reduction-smoke, and family-scaffold paths compile and load from this tree, then validate exported family and output-count metadata |
...
| JIT source generation | `src/JitML/Codegen/{RuntimeSource,Cuda,OneDnn,Metal,SourceFile}.hs`, `src/JitML/Engines/{HasEngine,Local}.hs`, `./.build/jit-src/<substrate>/<hash>/` | Haskell-generated compiler inputs for CUDA, oneDNN C++, and Metal / Swift; local Linux CPU identity, reduction-smoke, and family-scaffold compile/load/run plus family/output-count symbol validation, local Linux CPU `HasEngine` dispatch, and host/container artifact ABI separation in the Linux CPU fingerprint |
