...
| Route registry | `src/JitML/Routes.hs` — single source of truth for every HTTPRoute resource; Phase `4` routes target the actual Helm release services and send Harbor's public portal/API/registry/token paths through the chart's public `harbor` service | ✅ Done | Sprint 3.4 |
...
|-----------|---------------|---------|--------|---------------|
| Harbor (image registry) | `harbor` subchart values + typed `helm install` plan; `JitML.Service.HarborSubprocess` provides explicit Docker/curl-backed `HasHarbor` settings and commands | `/harbor` (portal), `/harbor/api` (API), `/v2` (Docker registry), `/service` (token service), all through the chart's public `harbor` nginx service | 🔄 Active; live Harbor rollout readiness, typed command rendering, and live push/pull/list/artifact-existence through `HasHarbor` are validated; missing: live S3-backend verification against MinIO | Sprint 4.1 |
| Percona PG operator | `pg-operator` subchart values plus local PV templates and `JitML.Cluster.PostgresRegistry` `PerconaPGCluster` rendering | (internal — packaged services that require Postgres) | 🔄 Active; live `harbor-pg` reconciliation and database readiness are validated; missing: Harbor Helm values wired to the live external database endpoint | Sprint 4.2 |
...
| MinIO (object store) | `minio` values and bucket list live in `chart/values.yaml`; the direct live install uses `chart/values/minio.yaml`; `src/JitML/Storage/Buckets.hs` is the typed bucket registry; `JitML.Cluster.Readiness.minioBucketReadinessSubprocess` checks every typed bucket through the in-pod Bitnami `mc` client | `/minio/console`, `/minio/s3` | 🔄 Active; live Linux CPU validation confirms MinIO rollout readiness and all seven typed buckets; missing: live `HasMinIO` conditional-write client | Sprint 4.3 |
| Apache Pulsar | `pulsar` HA values and topic-command renderer; direct live install uses `chart/values/pulsar.yaml`; topic creation runs an idempotent `/pulsar/bin/pulsar-admin topics list` / `topics create` script from `pulsar-toolset-0` | `/pulsar/admin`, `/pulsar/ws` | 🔄 Active; live Linux CPU bootstrap validates broker/bookkeeper/proxy readiness and topic creation; missing: live `HasPulsar` client and daemon subscription semantics | Sprint 4.4 |
...
| kube-prometheus-stack | `kube-prometheus-stack` values plus Grafana/Prometheus renderers | `/grafana`, `/prometheus` | 🔄 Active; live Prometheus + Grafana rollout readiness is validated; missing: Grafana dashboard route inspection and Prometheus scraping the daemon's `/metrics` endpoint | Sprint 4.5 |
| TensorBoard | TensorBoard event-key/projection/deployment renderer | `/tensorboard` | 🔄 Active; live Linux CPU bootstrap validates TensorBoard rollout readiness from `tensorflow/tensorflow:2.16.1`; missing: live MinIO event storage and live TensorBoard event reads | Sprint 4.6 |
...
| NVIDIA RuntimeClass (CUDA) | `chart/templates/runtimeclass-nvidia.yaml`; binds to nodes labelled `jitml.runtime/gpu=true` | (RuntimeClass — selected by the Linux CUDA Deployment podSpec) | 🔄 Active; RuntimeClass apply/get is validated in live Linux CPU bootstrap; missing: live binding validated on a Kind worker with the `nvidia` runtime + real GPU | Sprint 4.7 |
...
| Capability classes (`HasMinIO`, `HasPulsar`, `HasHarbor`, `HasKubectl`) — full doctrine-required methods checked in | Capability Classes and Service Errors | 🔄 Active; `HasHarbor` and `HasKubectl` subprocess instances are live-validated; missing: live MinIO and Pulsar instances plus daemon wiring for all clients | Sprint 5.4 |
