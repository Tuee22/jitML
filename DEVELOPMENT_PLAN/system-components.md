...
Phases `4`, `5`, `7`, `8`, `9`, `10`, `11`, and `12` are
...
| Service daemon | `jitml service` | Renders daemon lifecycle, BootConfig/LiveConfig, endpoint, logging, retry, at-least-once helper surfaces, BootConfig-derived client acquisition settings, BootConfig-derived daemon Pulsar subscription plans, per-topic startup subscription-acquisition status, combined `DaemonServiceClient` capability interpreter, bounded acquired-subscription consumer batch, and LiveConfig-derived dedup cache sizing; starts the in-binary HTTP listener, and the local Helm chart carries the current typed Dhall ConfigMap surface | 🔄 Active; live single-pod `/healthz`, `/readyz`, and `/metrics` are validated, local startup subscription acquisition crosses the typed `HasPulsar.pulsarSubscribe` boundary through `DaemonServiceClient`, and the bounded daemon batch is validated against a synthetic broker; missing: Pulsar long-lived subscription/redelivery from the running daemon, live MinIO put/CAS from the running daemon, live Harbor pull from the running daemon, live kubectl call from the running daemon | Sprints 5.1, 5.4, 5.5, 5.6 |
...
| `jitml-service` Deployment | `chart/templates/deployment-jitml-service.yaml`; local chart ConfigMap uses the current typed Dhall union constructors for residency/inference mode and current `LiveConfig` fields; Deployment uses required hostname anti-affinity | (internal target Pulsar consumer) | 🔄 Active; live single-replica Deployment readiness plus `/healthz`, `/readyz`, and `/metrics` are validated; live two-replica anti-affinity on a two-worker Kind cluster is validated; missing: real long-lived Pulsar subscription from the running pod | Sprint 5.6 |
...
| Docker assets | `docker/Dockerfile`, `docker/compose.yaml` | One Dockerfile producing one image (`jitml:local`) with `jitml`, `jitml-demo`, and the style-tool/code-quality gate; one compose service (`jitml`) |
