...
Phase `4` remains reopened and `⏸️ Blocked` after the local Kind topology moved
to a single-node cluster for every substrate; Phase `3` reclosed on 2026-05-23
after live Linux CPU bootstrap and teardown validated that topology, while the
Phase `4` live CUDA RuntimeClass probe now waits for a Linux CUDA validation
host with Docker's NVIDIA runtime and `nvidia-smi`. Phase `5` remains `🔄 Active`
for daemon service-pod validation on the same topology. Phases `7`, `8`, `9`,
`10`, `11`, and `12`
...
| Linux CUDA | `linux-cuda` | generated CUDA C source under `./.build/jit-src/linux-cuda/<hash>/`; host SplitMix64 RNG surface, generated no-curand metadata, deterministic reduction source with no `atomicAdd`, host reduction partial-count/finalization helpers, typed CUDA runtime probes for `nvcc`, `nvidia-smi`, and dynamic-linker visibility, and generated family/output-count metadata symbols | fully containerized target: `jitml:local` (CUDA activates at runtime when scheduled to `runtimeClassName: nvidia`) | target one daemon: `Cluster + SelfInference`, pod anti-affinity = one per node; current code renders configs and summaries; live `RuntimeClass/nvidia` GPU visibility must be repeated on the single-node Kind topology | 🔄 Active; missing: CUDA compile/load, cuBLAS/cuDNN execution, and refreshed single-node CUDA live validation | [Phase 7](phase-7-jit-codegen-and-substrates.md) |
...
| `jitml-service` Deployment | `chart/templates/deployment-jitml-service.yaml`; local chart ConfigMap uses the current typed Dhall union constructors for residency/inference mode and current `LiveConfig` fields; Deployment uses required hostname anti-affinity, `maxSurge: 0` / `maxUnavailable: 1`, and a `jitml-service` service account/RoleBinding | (internal target Pulsar consumer) | 🔄 Active; rendering, Dhall, service account/RBAC, single-node replacement rollout shape, and local dispatcher coverage are in place, but Sprint `5.6` must repeat live single-node Linux CPU readiness/consumer validation, Linux CUDA service-pod RuntimeClass validation on `jitml-linux-cuda-control-plane`, and Apple Silicon host Dhall subscription validation | Sprint 5.6 |
...
| NVIDIA RuntimeClass (CUDA) | `chart/templates/runtimeclass-nvidia.yaml`; `JitML.Cluster.Kind` renders the Linux CUDA single-node containerd `nvidia` runtime handler, repo-owned NVIDIA runtime config, read-only `/run/nvidia/driver` host driver-root mount, and toolkit support mounts | (RuntimeClass — selected by the Linux CUDA Deployment podSpec) | ⏸️ Blocked; manifest rendering, dry-run, chart lint, and integration coverage are updated for the single-node Linux CUDA config, but Sprint `4.7` needs a Linux CUDA validation host with Docker's NVIDIA runtime and `nvidia-smi` before the live `RuntimeClass/nvidia` probe can run; Sprint `5.6` must then repeat the live CUDA service-pod validation on `jitml-linux-cuda-control-plane` | Sprint 4.7 / Sprint 5.6 |
...
| Demo executable shim (`jitml-demo`) | `app/Demo.hs` shim into `App.demoMain`; `demoMain` prints `demoStatusLine` and starts `WebServer.serveDemo`, which serves the full local API route family and `/bundle/main.js` when `web/dist/Main/index.js` exists | 🔄 Active; missing: demo image bundle build and live WebSocket proxy to the daemon | Sprint 11.5 |
